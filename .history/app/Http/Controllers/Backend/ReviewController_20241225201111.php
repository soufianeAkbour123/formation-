...
class ReviewController extends Controller
{
    public function ShowReviewForm($courseId)
    {
        ...
        // Vérifier si l'utilisateur a assisté à au moins trois séances
        $attendedSessions = CourseSection::where('course_id', $courseId)
            ->where('is_cancelled', false)
            ->where('date', '<=', now()) // Les séances passées
            ->count();
    
        if ($attendedSessions < 2) {
            // L'utilisateur est inscrit mais n'a pas assisté à 3 séances
            return view('frontend.review.add_review', [
                'course' => null,
                'message' => 'Vous devez avoir assisté à au moins trois séances pour laisser un avis.',
                'canSubmit' => false,
            ]);
        }
        ...
        return view('frontend.review.add_review', [
            'course' => $course,
            'message' => null, // Pas de message d'erreur
