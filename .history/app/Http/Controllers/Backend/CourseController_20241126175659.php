...
public function UpdateCoursePdf(Request $request){
    ...
    return redirect()->back()->with($notification); 
            ...
            {
                \Log::info('Données reçues pour nouvelle section:', $request->all());
        ...
        \Log::info('Paramètres de date et heure:', [
        ...
        // Vérification des chevauchements
         ...
         if ($overlappingSections->isNotEmpty()) {
             \Log::warning('Conflit détecté:', [
                 ...
                 'sections_en_conflit' => $overlappingSections->toArray()
             ]);
             ...
             return response()->json([
                 'success' => false,
                 'message' => "Conflit horaire détecté. Séances existantes: $conflictTimes"
             ], 422);
         }
...
private function getOverlappingSections($courseId, $startTime, $endTime, $date)
{
    ...
    \Log::info('Vérification des chevauchements:', [
        'courseId' => $courseId,
        'date' => $date->toDateString(),
        'startTime' => $startTime->format('H:i'),
        'endTime' => $endTime->format('H:i')
    ...
    // Récupérer les sections existantes pour debug
    $existingSections = CourseSection::where('course_id', $courseId)
        ->where('date', $date->toDateString())
        ->get();

    \Log::info('Sections existantes pour cette date:', $existingSections->toArray());

    // Vérification des chevauchements avec logging
    $overlappingSections = CourseSection::where('course_id', $courseId)
        ->where('date', $date->toDateString())
