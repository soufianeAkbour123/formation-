...
public function PromoApply(Request $request)
{
    ...
    if ($promo) {
        // Appliquer la logique du code promo (calculer la réduction, etc.)
        if ($course->promo_id == $promo->id) {
            $discountAmount = $promo->discount_amount;
            $totalAmount = $course->price - $discountAmount;

            // Mettre à jour la session avec les informations du code promo
            session()->put('promo', [
                'code' => $promoCode,
            ...
            return response()->json([
                'validity' => true,
                'message' => 'Le code est valide pour ce cours',
            ]);
        } else {
            return response()->json([
                'validity' => false,
                'message' => 'Ce code promo n\'est pas valide pour ce cours',
            ]);
        }
    } else {
        return response()->json([
...
public function PromoCalculation() {
    try {
        ...
        if (Session::has('promo')) {
            ...
            if (!isset($promoSession['discount']) || !isset($promoSession['code'])) {
                return response()->json([
                    'error' => 'Invalid promo session',
                    'subtotal' => $subtotal,
                    'total' => $subtotal,
                ], 400);
            }

            $discountPercentage = $promoSession['discount'];
            $discountAmount = ($subtotal * $discountPercentage) / 100;
            $totalAmount = $subtotal - $discountAmount;

            session()->put('promo', [
                'code' => $promoSession['code'],
