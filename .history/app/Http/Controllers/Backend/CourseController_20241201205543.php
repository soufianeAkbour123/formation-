        ...
        public function rescheduleSection(Request $request)
{
    try {
        ...
        // Vérifier si c'est une séance déjà reprogrammée deux fois
        $rescheduledCount = CourseSection::where('original_section_id', 
            $originalSection->original_section_id ?? $originalSection->id)
        ...
        if ($rescheduledCount >= 2) {
            return response()->json([
                'success' => false,
                'message' => 'Cette séance ne peut plus être reprogrammée',
            ], 422);
        }
        ...
        // Gérer la référence à la section originale
        if ($originalSection->original_section_id) {
            $newSection->original_section_id = $originalSection->original_section_id;
        } else {
