...
public function SaveLecture(Request $request)
{
    try {
        ...
        return response()->json([
            ...
            {
                // Validation des champs principaux avec règles de validation pour les lecteurs
                $validator = Validator::make($request->all(), [
                    ...
                    'chapters.*.title' => 'sometimes|string|max:255' // Validation pour les titres de lecteurs
                ], [
                    ...
                    'chapters.*.title.max' => 'Le titre d\'un chapitre ne peut pas dépasser 255 caractères.'
                ]);
                ...
                try {
                    ...
                    // Ajouter les chapitres à la nouvelle section
                    if ($request->has('chapters') && is_array($request->chapters)) {
                        foreach ($request->chapters as $chapterData) {
                            // Vérifier que le titre du chapitre est non vide
                            if (!empty($chapterData['title'])) {
                                $lecture = new CourseLecture();
                                $lecture->course_section_id = $newSection->id;
                                $lecture->lecture_title = $chapterData['title'];
                                $lecture->save();
                            }
                        }
                    }
                    ...
                    return response()->json([
                        'status' => 'error',
                        'message' => 'Erreur lors de la reprogrammation : ' . $e->getMessage()
                    ], 500);
                }
            }
        }
