...
public function SaveLecture(Request $request)
{
    try {
        ...
        return response()->json([
            ...
            {
                \Log::info('Reschedule Section Request Data:', $request->all());
                ...
                try {
                    ...
                    // Create a new section as a copy of the original
                    $rescheduledSection = $originalSection->replicate();
                    
                    // Set new details for the rescheduled section
                    $rescheduledSection->date = $request->date;
                    $rescheduledSection->start_time = Carbon::createFromFormat('H:i', $request->start_time);
                    $rescheduledSection->end_time = Carbon::createFromFormat('H:i', $request->end_time);
                    $rescheduledSection->section_title = $originalSection->section_title . ' (Reprogrammé)';
                    $rescheduledSection->is_rescheduled = true;
                    $rescheduledSection->original_section_id = $originalSection->id;
                    
                    // Keep the original section as cancelled
                    $originalSection->is_cancelled = true;
                    $originalSection->cancellation_reason = 'Séance reprogrammée le ' . $request->date;
                    
                    // Save both sections
                    $originalSection->save();
                    $rescheduledSection->save();
                    ...
                    \Log::error('Reschedule Section Error: ' . $e->getMessage());
                    return response()->json([
                        'success' => false,
                        'message' => 'Impossible de reprogrammer la séance.',
