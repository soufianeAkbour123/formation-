...
public function SaveLecture(Request $request)
{
    try {
        ...
        return response()->json([
            ...
            {
                $originalSection = Section::findOrFail($request->section_id);
                
                // Mark original section as cancelled
                $originalSection->update([
                    'is_cancelled' => true,
                    'cancellation_reason' => 'Reprogrammée le ' . now()->format('d/m/Y')
                ]);
                
                // Create new section
                $newSection = Section::create([
                    'course_id' => $originalSection->course_id,
                    'section_title' => $request->original_section_title . ' (Reprogrammé)',
                    'date' => $request->date,
                    'start_time' => $request->start_time,
                    'end_time' => $request->end_time,
                    'is_rescheduled' => true,
                    'original_section_id' => $originalSection->id
                ]);
