...
public function calendar()
{
    ...
    $sections = CourseSection::with('lectures')
        ...
        ->map(function ($section) use ($now) {
            $sessionStartTime = Carbon::parse($section->date . ' ' . $section->start_time);
            $sessionEndTime = Carbon::parse($section->date . ' ' . $section->end_time);
            ...
            return [
                ...
                'is_rescheduled' => $section->is_rescheduled ?? false, // Assurez-vous que ce champ existe
                ...
                'is_past_session' => $now->gt($sessionEndTime),
                'is_ongoing_session' => $now->between($sessionStartTime, $sessionEndTime)
            ];
        });
