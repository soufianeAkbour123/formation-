...
public function calendar()
{
    ...
    // 1. Récupérer les IDs des cours auxquels l'utilisateur est inscrit avec un paiement confirmé
    $enrolledCourseIds = Order::join('payments', 'orders.payment_id', '=', 'payments.id')
        ->where('orders.user_id', $userId)
        ->where('payments.status', 'confirm')  // Assurez-vous que le paiement est confirmé
        ->pluck('orders.course_id')
        ->toArray();

    // 2. Si l'utilisateur n'est inscrit à aucun cours, retourner une collection vide
    if (empty($enrolledCourseIds)) {
        return view('frontend.mycourse.event', ['sections' => collect([])]);
    }

    // 3. Récupérer les sections des cours auxquels l'utilisateur est inscrit
    $sections = CourseSection::with(['CourseLecture', 'course'])
        ...
        ->map(function ($section) use ($now) {
            return [
                ...
                'course_name' => $section->course->title ?? 'N/A', // Ajout du nom du cours
