...
public function calendar()
{
    ...
    // 1. D'abord, vérifions si l'utilisateur a des commandes confirmées
    $hasConfirmedOrders = Order::join('payments', 'orders.payment_id', '=', 'payments.id')
        ->where('orders.user_id', $userId)
        ->where('payments.status', 'confirm')
        ->exists();

    // 2. Si pas de commandes confirmées, retourner immédiatement
    ...
    // 3. Si l'utilisateur a des commandes, récupérer les sections uniquement pour ses cours
    $userCourseIds = Order::join('payments', 'orders.payment_id', '=', 'payments.id')
        ->where('orders.user_id', $userId)
        ->where('payments.status', 'confirm')
        ->pluck('orders.course_id')
        ->toArray();

    // 4. Récupérer les sections uniquement pour les cours achetés
    $sections = CourseSection::whereIn('course_id', $userCourseIds)
        ...
        ->map(function ($section) {
            return [
                ...
                'lectures_count' => 0 // ou le compte réel si nécessaire
            ];
        });
    ...
    \Log::info('Calendar Debug', [
        'userId' => $userId,
        'hasConfirmedOrders' => $hasConfirmedOrders,
