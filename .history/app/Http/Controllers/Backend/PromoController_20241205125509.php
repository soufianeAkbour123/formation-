...
{
    // Vérifier si le code promo a été saisi
    ...
    $cartTotal = Cart::total(null, null, '');
    ...
    return response()->json([
        ...
        'subtotal' => number_format($cartTotal, 2, ',', ' ') . ' DHS',
        'promo_code' => $promo->code,
        'promo_discount' => $discount,
        'discount_amount' => number_format($discountAmount, 2, ',', ' ') . ' DHS',
        'total_amount' => number_format($totalAmount, 2, ',', ' ') . ' DHS'
    ]);
}
...
public function PromoCalculation()
{
    ...
    if (Session::has('promo')) {
        ...
        return response()->json([
            ...
            'discount_amount' => number_format($promoData['discount_amount'], 2, ',', ' ') . ' DHS',
            'total_amount' => number_format($promoData['total_amount'], 2, ',', ' ') . ' DHS'
        ]);
    }

    return response()->json([
        'subtotal' => number_format($cartTotal, 2, ',', ' ') . ' DHS',
        'total' => number_format($cartTotal, 2, ',', ' ') . ' DHS'
    ]);
}
