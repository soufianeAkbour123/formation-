        ...
        return response()->json([
            ...
            public function rescheduleSection(Request $request)
            {
                try {
                    ...
                    // Si c'est une séance initiale annulée
                    if (!$originalSection->is_rescheduled) {
                        // Première reprogrammation
                        ...
                        $newSection->cancellation_reason = null;
                    ...
                    // Si c'est une séance reprogrammée et annulée
                    if ($originalSection->is_rescheduled && $originalSection->is_cancelled) {
                        // Vérifier si la séance originale existe
                        $parentSection = CourseSection::find($originalSection->original_section_id);
                        if (!$parentSection) {
                            return response()->json([
                                'success' => false,
                                'message' => 'Séance originale introuvable'
                            ], 422);
                        }
            
                        // Créer une nouvelle reprogrammation
                        $newSection = $originalSection->replicate();
                        $newSection->date = $request->date;
                        $newSection->start_time = $request->start_time;
                        $newSection->end_time = $request->end_time;
                        ...
                        $newSection->is_cancelled = false;
                        $newSection->cancellation_reason = null;
                        $newSection->original_section_id = $parentSection->id;
                        $newSection->save();
                    ...
                    return response()->json([
                        'success' => false,
                        'message' => 'Cette séance ne peut pas être reprogrammée'
                    ], 422);
                    ...
                    return response()->json([
                        'success' => false,
                        'message' => 'Erreur lors de la reprogrammation de la séance: ' . $e->getMessage()
                    ], 500);
                }
            }
        }
