    ...
    return redirect()->back()->with($notification); 
            ...
            public function AddCourseSection(Request $request)
            {
                try {
                    ...
                    // Création de la section directement avec les données brutes
                    ...
                    return response()->json([
                        'success' => false,
                        'message' => $e->getMessage(), // Afficher le message d'erreur réel
          ...
          public function rescheduleSection(Request $request)
{
    try {
        ...
        // Vérifier si la section est déjà reprogrammée
        if ($originalSection->is_rescheduled) {
            return response()->json([
                'success' => false,
                'message' => 'Cette séance a déjà été reprogrammée',
            ], 422);
        }
        ...
        // Créer une nouvelle section avec les détails reprogrammés
        ...
        $newSection->is_cancelled = false;
        $newSection->save();
        
        // Copier les chapitres à la nouvelle section
        ...
        // Mettre à jour l'ancienne section
        $originalSection->update([
            'is_cancelled' => true,
            'is_rescheduled' => true, // Marquer comme reprogrammée
            'cancellation_reason' => 'Séance reprogrammée'
        ]);
        ...
        return response()->json([
            'success' => false,
            'message' => 'Erreur lors de la reprogrammation de la séance'
        ], 500);
    }
}
public function searchCourses(Request $request) 
{
    ...
    return view('frontend.course.course_view', compact('courses','categories','levels','ratings'));
}
...
public function filterByLevel($level)
{
    ...
    if($level !== 'all') {
        $courses = $courses->where('label', $level);
    }
...
{
    $courses = Course::where('status', 1)
        ->whereHas('reviews', function($query) use ($rating) {
...
{
    return [
        'all' => Course::where('status', 1)->count(),
        'Begginer' => Course::where('status', 1)->where('label', 'Begginer')->count(),
        'Middle' => Course::where('status', 1)->where('label', 'Middle')->count(),
        'Advance' => Course::where('status', 1)->where('label', 'Advance')->count()
    ];
}
...
{
    return [
        5 => Course::whereHas('reviews', function($q) { $q->where('rating', 5); })->count(),
        4 => Course::whereHas('reviews', function($q) { $q->where('rating', '>=', 4); })->count(),
        3 => Course::whereHas('reviews', function($q) { $q->where('rating', '>=', 3); })->count(),
        2 => Course::whereHas('reviews', function($q) { $q->where('rating', '>=', 2); })->count(),
        1 => Course::whereHas('reviews', function($q) { $q->where('rating', '>=', 1); })->count(),
    ];
}
public function category($id, $slug)
{
    ...
    return view('frontend.course.course_view', compact('category', 'courses', 'categories', 'levels', 'ratings'));
}
        }
