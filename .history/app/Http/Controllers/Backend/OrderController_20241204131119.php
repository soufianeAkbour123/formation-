...
public function calendar()
{
    ...
    // 1. Déboguer les commandes de l'utilisateur
    $orderedCourseIds = Order::where('user_id', $userId)
                            ->where('status', 'active') // Ajout d'une vérification du statut si nécessaire
                            ->pluck('course_id');

    // 2. Vérifier si l'utilisateur a des commandes
    if ($orderedCourseIds->isEmpty()) {
        // Si l'utilisateur n'a pas de commandes, retourner une collection vide
        return view('frontend.mycourse.event', ['sections' => []]);
    }

    // 3. Déboguer les sections trouvées
    ...
    // 5. Ajouter une vérification dans la vue
    return view('frontend.mycourse.event', [
        'sections' => $formattedSections,
        'hasOrders' => !$orderedCourseIds->isEmpty()
    ]);
}
