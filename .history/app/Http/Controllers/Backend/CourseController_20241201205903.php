        ...
        return response()->json([
            ...
            public function rescheduleSection(Request $request)
            {
                try {
                    ...
                    // Vérifier si la séance est annulée et n'a jamais été reprogrammée
                    if (!$originalSection->is_cancelled || $originalSection->is_rescheduled) {
                        return response()->json([
                            'success' => false,
                            'message' => 'Cette séance ne peut pas être reprogrammée. Seule une séance annulée qui n\'a jamais été reprogrammée peut être reprogrammée.',
                        ], 422);
                    }
                    ...
                    $validator = Validator::make($request->all(), [
                        ...
                        'end_time' => [
                            ...
                            function ($attribute, $value, $fail) {
                                $hour = (int) substr($value, 0, 2);
                                if ($hour >= 23) {
                                    $fail('Les cours doivent se terminer avant 23:00');
                                }
                            },
                        ],
                    ]);
                    ...
                    $newSection->course_id = $originalSection->course_id;
                    $newSection->section_title = $originalSection->section_title;
                    $newSection->note = $originalSection->note;
                    $newSection->link = $originalSection->link;
                    ...
                    $newSection->original_section_id = $originalSection->id;
                    $newSection->save();
            
                    // Copier les chapitres
                    foreach ($originalSection->lectures as $lecture) {
                        $newLecture = new CourseLecture();
                        $newLecture->course_id = $lecture->course_id;
                        $newLecture->section_id = $newSection->id;
                        $newLecture->lecture_title = $lecture->lecture_title;
                        $newLecture->save();
                    }
            
                    // Mettre à jour la section originale
                    ...
                    \Log::error('Erreur de reprogrammation: ' . $e->getMessage());
                    return response()->json([
                        'success' => false,
