...
public function calendar()
{
    ...
    // Récupérer d'abord les commandes de l'utilisateur avec leurs cours
    $userOrders = Order::where('user_id', $userId)
        ->with('course') // Assurez-vous d'avoir la relation définie dans le modèle Order
        ->get();

    // Récupérer les IDs des cours
    $enrolledCourseIds = $userOrders->pluck('course_id')->unique();

    // Récupérer les sections des cours
    $sections = CourseSection::with(['course', 'CourseLecture'])
