        ...
        return response()->json([
            ...
            public function rescheduleSection(Request $request)
            {
                try {
                    ...
                    'end_time' => 'required',
                    'link' => 'required|url',
                    'lectures' => 'nullable|string'
                ]);
                \Log::info('Données reçues pour reprogrammation', [
                    'data' => $request->all(),
                    'lectures' => $request->lectures
                ]);
            
                // Récupérer la section originale
                $originalSection = CourseSection::findOrFail($request->original_section_id);
            
                // Créer une nouvelle section
                $newSection = new CourseSection();
                $newSection->course_id = $originalSection->course_id;
                $newSection->section_title = $request->section_title;
                $newSection->date = $request->date;
                $newSection->start_time = $request->start_time;
                $newSection->end_time = $request->end_time;
                $newSection->link = $request->link;
                $newSection->save();
            
                // Gérer les lectures
                $lecturesData = json_decode($request->lectures, true);
                if ($lecturesData) {
                    foreach ($lecturesData as $lectureInfo) {
                        $originalLecture = Lecture::findOrFail($lectureInfo['id']);
                        $newLecture = $originalLecture->replicate();
                        $newLecture->section_id = $newSection->id;
                        $newLecture->lecture_title = $lectureInfo['title'];
                        $newLecture->save();
                    }
                }
            
                // Supprimer la section originale
                $originalSection->delete();
            
                return response()->json(['message' => 'Séance reprogrammée avec succès']);
            } } catch (\Exception $e) {
                // Log détaillé de l'erreur
                \Log::error('Erreur de reprogrammation : ', [
                ...
                return response()->json([
                    'error' => 'Erreur de reprogrammation',
                    'message' => $e->getMessage()
                ], 500);
            }
        }
            // Dans votre CourseController ou SectionController
public function getSectionDetails($sectionId)
{
    try {
        // Charger la section avec ses lectures
        $section = CourseSection::with('lectures')
            ->where('id', $sectionId)
            ->first();
        ...
        // Log de l'erreur
        \Log::error('Erreur de récupération de la section : ' . $e->getMessage());
