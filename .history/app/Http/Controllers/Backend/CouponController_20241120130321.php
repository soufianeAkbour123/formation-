...
class CouponController extends Controller
{
    ...
    use App\Models\Category; // Assurez-vous d'importer Category
    use Carbon\Carbon;
    use Illuminate\Support\Str;
    ...
    class CouponController extends Controller
    {
        ...
        {
            // Vérifiez que le coupon existe
            $coupon = Coupon::findOrFail($id);
            
            // Récupérez toutes les catégories avec leurs cours
            $categories = Category::with('courses')->get();
            
            // Récupérez les IDs des cours déjà associés à ce coupon
            $selectedCourses = $coupon->courses()->pluck('courses.id')->toArray();
            
            // Passez toutes les variables nécessaires à la vue
            return view('admin.backend.coupon.select_courses', [
                'coupon' => $coupon,
    ...
    public function updateCourses(Request $request, $couponId)
    {
        $coupon = Coupon::findOrFail($couponId);
        $coupon->courses()->sync($request->courses);

        return redirect()->route('admin.all.coupon')->with([
            'message' => 'Formations associées au coupon mises à jour avec succès',
            'alert-type' => 'success'
        ]);
    }
    ...
    public function validateCoupon(Request $request)
    {
        ...
        if (!$coupon) {
            return response()->json(['valid' => false, 'message' => 'Coupon invalide']);
        }

        // Vérifier si le coupon est applicable au cours
        $courseId = $request->course_id;
        if (!$coupon->courses->contains($courseId)) {
            return response()->json([
                'valid' => false,
                'message' => 'Ce coupon n\'est pas applicable à cette formation'
            ]);
        }
