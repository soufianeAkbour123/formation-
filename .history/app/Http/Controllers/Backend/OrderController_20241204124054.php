...
public function calendar()
{
    ...
    // Vérifier d'abord si l'utilisateur a des commandes
    $hasOrders = Order::where('user_id', $userId)->exists();

    // Si l'utilisateur n'a pas de commandes, retourner un tableau vide
    if (!$hasOrders) {
        $sections = collect([]); // Collection vide
        return view('frontend.mycourse.event', compact('sections'));
    }

    // Si l'utilisateur a des commandes, continuer avec la logique existante
