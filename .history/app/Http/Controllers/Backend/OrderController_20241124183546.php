...
{
    $sections = CourseSectionSection::all(); // Récupérez vos données des sections.
    $events = [];

    foreach ($sections as $sec) {
        $events[] = [
            'title' => $sec->section_title,
            'start' => Carbon::parse($sec->date . ' ' . $sec->start_time)->toIso8601String(),
            'end' => Carbon::parse($sec->date . ' ' . $sec->end_time)->toIso8601String(),
            'backgroundColor' => $sec->is_cancelled ? '#ff4d4d' : ($sec->date == today() ? '#34c38f' : '#3778ff'),
            'borderColor' => $sec->is_cancelled ? '#ff4d4d' : '#3778ff',
            'url' => $sec->link,
        ];
    }
