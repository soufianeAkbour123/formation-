...
public function SaveLecture(Request $request)
{
    try {
        ...
        return response()->json([
             ...
             * Reprogrammer une section de cours
     * 
     * @param array $data Données de reprogrammation
     * @return CourseSection La nouvelle section créée
     * @throws SectionReschedulingException
     */
    public function rescheduleSection(array $data)
    {
        // Vérifier les permissions (à implémenter selon votre logique d'authentification)
        // if (!Auth::user()->canRescheduleSection($data['original_section_id'])) {
        //     throw new SectionReschedulingException('Vous n\'avez pas les permissions nécessaires.');
        // }

        // Vérifier les conflits de programmation
        $this->checkSectionConflicts($data);

        DB::beginTransaction();
        try {
            // Récupérer la section originale
            $originalSection = CourseSection::findOrFail($data['original_section_id']);

            // Créer la nouvelle section
            $newSection = $this->createNewSection($originalSection, $data);

            // Copier les lectures de la section originale
            $this->copyLectures($originalSection, $newSection);

            // Marquer la section originale comme reprogrammée si nécessaire
            $originalSection->is_rescheduled = true;
            ...
            DB::rollBack();
            throw new SectionReschedulingException(
                'Erreur lors de la reprogrammation : ' . $e->getMessage()
            );
        }
    }
     ...
     * @throws SectionReschedulingException
     */
    private function checkSectionConflicts(array $data)
    {
        $conflictingSections = CourseSection::where('course_id', $data['course_id'])
        ...
        if ($conflictingSections) {
            throw new SectionReschedulingException('Une autre section est déjà programmée à ce moment.');
        }
    }
     ...
     * 
     * @param CourseSection $originalSection
     * @param array $data
     * @return CourseSection
     */
    private function createNewSection(CourseSection $originalSection, array $data)
    {
        return CourseSection::create([
            'course_id' => $originalSection->course_id,
            'section_title' => $data['section_title'] ?? $originalSection->section_title,
            'date' => $data['date'],
            'start_time' => $data['start_time'],
            'end_time' => $data['end_time'],
            'link' => $data['link'],
            'is_cancelled' => false,
            'parent_section_id' => $originalSection->id // Garder une trace de la section originale
        ]);
    }
     ...
     * 
     * @param CourseSection $originalSection
     * @param CourseSection $newSection
     */
    private function copyLectures(CourseSection $originalSection, CourseSection $newSection)
    {
        $originalLectures = $originalSection->lectures;
...
class SectionReschedulingException extends \Exception {}
...
class CourseSectionController extends Controller
{
    ...
    {
        $validator = Validator::make($request->all(), [
            'original_section_id' => 'required|exists:course_sections,id',
            'course_id' => 'required|exists:courses,id',
        ...
        try {
            $newSection = $this->sectionService->rescheduleSection($validator->validated());
        ...
        } catch (SectionReschedulingException $e) {
            return response()->json([
                'status' => 'error',
