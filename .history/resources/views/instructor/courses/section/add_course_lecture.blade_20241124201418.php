                            ...
                            <div class="section-header-content">
                                <div class="section-main-info">
                                    ...
                                    <div class="action-buttons">
                                        ...
                                        @else
                                            <!-- Bouton Ajouter -->
                                        <button class="btn btn-sm btn-primary" onclick="addLecture({{ $course->id }}, {{ $item->id }})">
                                            <i class="las la-plus"></i>Chapitre
                                        </button>

                                        <!-- Bouton Vidéos enregistrées -->
                                        ...
                                        <button class="btn btn-sm btn-warning cancel-section" 
                                            ...
                                            <!-- <button class="btn btn-warning postpone-section" 
                                                data-id="{{ $item->id }}" 
                                                data-title="{{ $item->section_title }}">
                                                <i class="las la-clock"></i> Reporter
                                            </button> -->
                                        @endif
                                ...
                                <span class="value editable-link" data-id="{{ $item->id }}">
                                    <!-- Affiche une version raccourcie du lien -->
                                    <a href="{{ $item->link }}" target="_blank" title="{{ $item->link }}" class="link-text">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const errorManager = {
        ...
        showError: (input, message) => {
            ...
            if (!errorManager.errors.has(input) || errorManager.errors.get(input) !== message) {
                errorElement.textContent = message;
                input.classList.add('is-invalid');
                errorManager.errors.set(input, message);
            }
        },
        ...
        clearError: (input) => {
            ...
            errorManager.errors.delete(input);
        },
    ...
    const validators = {
        date: () => {
            ...
            errorManager.clearError(inputs.date);

            if (!inputs.date.value) {
                errorManager.showError(inputs.date, 'La date est requise');
                return false;
            }

            if (!/^\d{4}-\d{2}-\d{2}$/.test(inputs.date.value)) {
                errorManager.showError(inputs.date, 'Format de date invalide (YYYY-MM-DD)');
                return false;
            }

            if (selectedDate < today) {
                errorManager.showError(inputs.date, 'Impossible de planifier une séance dans le passé');
                return false;
            }
        ...
        times: () => {
            ...
            errorManager.clearError(startTime);
            errorManager.clearError(endTime);

            if (!startTime.value || !endTime.value) {
                !startTime.value && errorManager.showError(startTime, 'L\'heure de début est requise');
                !endTime.value && errorManager.showError(endTime, 'L\'heure de fin est requise');
                return false;
            }
            ...
            const [startHour, startMinutes] = startTime.value.split(':').map(Number);
            const [endHour, endMinutes] = endTime.value.split(':').map(Number);

            if (startHour < 8) {
                errorManager.showError(startTime, 'Les cours ne peuvent pas commencer avant 08:00');
                return false;
            }

            if (endHour >= 23 && endMinutes > 0) {
                errorManager.showError(endTime, 'Les cours doivent se terminer au plus tard à 23:00');
                return false;
            }

            if (endDateTime <= startDateTime) {
                errorManager.showError(endTime, 'L\'heure de fin doit être après l\'heure de début');
                return false;
            }

            const durationHours = (endDateTime - startDateTime) / (1000 * 60 * 60);
            if (durationHours > 8) {
                errorManager.showError(endTime, 'La durée d\'une séance ne peut pas dépasser 8 heures');
                return false;
            }
    ...
    // Gestionnaire d'événements unifié pour les inputs
    const handleInputChange = (input, validatorType) => {
        const debounceTimeout = input.dataset.debounceTimeout;
        clearTimeout(debounceTimeout);
        input.dataset.debounceTimeout = setTimeout(() => {
    ...
    Object.entries(inputs).forEach(([key, input]) => {
        ['input', 'change'].forEach(event => {
            input.addEventListener(event, () => 
                handleInputChange(input, key === 'date' ? 'date' : 'times'));
        });
    });

    // Gestionnaire de soumission du formulaire
  // Dans la partie fetch du formulaire
            ...
            if (data.success) {
                ...
                if (data.errors) {
                    Object.entries(data.errors).forEach(([field, messages]) => {
                        ...
                        if (input) {
                            errorManager.showError(input, messages[0]);
                        }
                    });
                } else if (data.message) {
                    // Identifier le champ concerné par l'erreur
                    const message = data.message.toLowerCase();
                    if (message.includes('passé') || message.includes('date')) {
                        errorManager.showError(inputs.date, data.message);
                    } else if (message.includes('créneau') || message.includes('horaire')) {
                        errorManager.showError(inputs.startTime, data.message);
                    } else {
                        // Si on ne peut pas associer l'erreur à un champ spécifique
                        Swal.fire({
                            title: 'Erreur',
                            text: data.message,
                            icon: 'error'
...
<script>
    ...
    formatDate(date, isForInput = true) {
        try {
            if (!date) return '';
            
            const regex = isForInput ? /^(\d{2})\/(\d{2})\/(\d{4})$/ : /^(\d{4})-(\d{2})-(\d{2})$/;
            const match = date.match(regex);
            if (!match) return isForInput ? '' : date;
            ...
            console.error(`Error formatting date (${isForInput ? 'input' : 'display'}):`, error);
            return isForInput ? '' : date;
        }
    }

    formatDateForInput = (date) => this.formatDate(date, true);
    formatDateForDisplay = (date) => this.formatDate(date, false);
    ...
    async startEditing(container) {
        ...
        try {
            ...
            if (!currentValue) throw new Error('Valeur actuelle non trouvée');
