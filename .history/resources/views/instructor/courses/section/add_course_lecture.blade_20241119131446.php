                        ...
                        <div class="section-body" id="section-body-{{ $item->id }}">
                            ...
                            <div id="lecture-list-{{ $item->id }}">
                                ...
                                <div class="lecture-item">
                                    ...
                                    <div class="action-buttons">
                                        <a href="{{ route('edit.lecture', $lecture->id) }}" class="btn btn-sm btn-primary">
        ...
        if (validators.date() && validators.times()) {
            try {
                ...
                if (data.success) {
                    ...
                    const input = data.message.toLowerCase().includes('créneau') ? inputs.startTime : inputs.date;
                    errorHandlers.showError(input, data.message);
                }
            } catch (error) {
                Swal.fire({
                    title: 'Erreur!',
...
<script>
    ...
    getSingleFieldValue(container) {
        const value = container.find('input').val()?.trim();
        if (!value) throw new Error('Le champ ne peut pas être vide.');
        
        if (this.config.fieldType === 'date') {
            this.validateDate(value);
        }
        
        return { [this.config.fieldName]: value, value };
    }
    ...
    async saveField(container) {
        ...
        try {
            ...
            const newValue = this.config.fieldType === 'time-range' 
                ? this.getTimeRangeValues(container) 
                : this.getSingleFieldValue(container);

            Object.entries(newValue).forEach(([key, value]) => formData.append(key, value));
