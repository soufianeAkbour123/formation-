...
<script>
 function rescheduleCourse(sectionId) {
    $.ajax({
        ...
        method: 'GET',
        dataType: 'json', // Spécifier explicitement le type de données
        success: function(section) {
            console.log('Section récupérée :', section); // Log détaillé

            // Vérifier si la section existe
            if (!section || !section.id) {
                Swal.fire('Erreur', 'Aucune donnée de séance trouvée', 'error');
                return;
            }
            ...
            if (section.date) {
                ...
                let formattedDate = section.date.split(' ')[0]; // Enlever l'heure si présente
            ...
            if (section.start_time) {
                $('#reschedule_start_time').val(
                    section.start_time.substr(0, 5) || 
                    section.start_time
                );
            }
            if (section.end_time) {
                $('#reschedule_end_time').val(
                    section.end_time.substr(0, 5) || 
                    section.end_time
                );
            }
            ...
            // Vérification supplémentaire pour les lectures
        ...
        error: function(xhr, status, error) {
            ...
            Swal.fire('Erreur de récupération', `
                Impossible de récupérer les détails de la séance. 
                Statut : ${xhr.status}
                Erreur : ${error}
            `, 'error');
        }
