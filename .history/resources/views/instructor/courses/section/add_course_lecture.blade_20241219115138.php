       ...
       <div class="col-12">
           ...
           <div class="card radius-10">
               <div class="card-body">
                   <div class="d-flex align-items-center">
                       ...
                       <button type="button" class="btn btn-secondary" onclick="window.open('https://teams.microsoft.com/v2/?meetingjoin=true', '_blank')">
           ...
           <div class="curriculum-content mt-4">
               <div class="accordion" id="courseAccordion">
                   ...
                   <div class="course-section {{ $item->is_cancelled ? 'cancelled-section' : '' }} {{ $item->is_rescheduled ? 'rescheduled-section' : '' }} {{ $item->is_postponed ? 'postponed-section' : '' }} {{ \Carbon\Carbon::parse($item->date)->isPast() ? 'past-section' : '' }}" id="section-container-{{ $item->id }}">
                       <div class="section-header" data-section="{{ $item->id }}">
                           @if($item->is_cancelled)
                           <div class="cancellation-badge">ANNULÉ</div>
                           <div class="cancellation-reason">Raison : {{ $item->cancellation_reason }}</div>
                            ...
                            <div class="section-header-content">
                               <div class="section-main-info">
                                   ...
                                   <div class="action-buttons">
                                       ...
                                       <button class="btn btn-sm btn-success restore-section" data-id="{{ $item->id }}" title="Restaurer la séance">
                                           <i class="las la-undo-alt"></i> Restaurer
                                       </button>
                                       <button class="btn btn-sm btn-warning reschedule-section" data-id="{{ $item->id }}" title="Reprogrammer la séance">
                                           <i class="las la-calendar-alt"></i> Reprogrammer
                                       </button>
                                       @endif
                                       <button class="btn btn-sm btn-danger delete-section" data-id="{{ $item->id }}" data-title="{{ $item->section_title }}">
                                       ...
                                       <button class="btn btn-sm btn-warning cancel-section" data-id="{{ $item->id }}" data-title="{{ $item->section_title }}">
                           ...
                           <div id="lecture-form-{{ $item->id }}" style="display: none;" class="mb-3 p-3 bg-white rounded">
                            ...
                            <div id="lecture-list-{{ $item->id }}">
                               ...
                               <div class="lecture-item">
                                   ...
                                   <div class="action-buttons">
                                       @if(!$item->is_cancelled && !\Carbon\Carbon::parse($item->date)->isPast())
                                       <button class="btn btn-sm btn-danger delete-lecture" data-id="{{ $lecture->id }}" data-title="{{ $lecture->lecture_title }}">
                                           <i class="las la-trash"></i>
                                       </button>
                                       @endif
                                   </div>
...
<script>
   ...
   function updateAfterCancel(sectionId, cancellationReason) {
       ...
       actionButtons.html(`
           ...
           <button class="btn btn-sm btn-warning reschedule-section" data-id="${sectionId}">
               <i class="las la-calendar-alt"></i> Reprogrammer
           </button>
    ...
    // Gestionnaire pour le formulaire d'annulation
               ...
               $.ajax({
                   ...
                   success: function(response) {
                       if (response.success) {
                           ...
                           Swal.fire({
                               title: 'Succès',
                               text: 'La séance a été restaurée',
                               icon: 'success',
                               timer: 1500
                           });
                       }
