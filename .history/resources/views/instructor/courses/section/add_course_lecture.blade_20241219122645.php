...
$(document).ready(function() {
    // Fonction pour mettre à jour l'interface après une reprogrammation
    function updateAfterReschedule(originalSectionId, newSectionId, newDate, newStartTime, newEndTime) {
        const originalSectionContainer = $(`#section-container-${originalSectionId}`);
        const newSectionContainer = $(`#section-container-${newSectionId}`);

        // Mettre à jour l'ancienne section pour indiquer qu'elle est annulée
        originalSectionContainer.addClass('cancelled-section');
        originalSectionContainer.find('.cancellation-badge').remove();
        originalSectionContainer.find('.section-header').prepend(`
            <div class="cancellation-badge">ANNULÉ</div>
            <div class="cancellation-reason">Raison : Séance reprogrammée</div>
        `);

        // Mettre à jour la nouvelle section avec les nouvelles informations
        newSectionContainer.find('.date-text').text(newDate);
        newSectionContainer.find('.start-time-text').text(newStartTime);
        newSectionContainer.find('.end-time-text').text(newEndTime);
        newSectionContainer.addClass('rescheduled-section');
        newSectionContainer.find('.section-title-text').append(' (Reprogrammé)');
    }

    // Gestionnaire pour la reprogrammation de section
    $('#rescheduleSectionForm').on('submit', function(e) {
        e.preventDefault();
        
        const sectionId = $('#reschedule_section_id').val();
        const newDate = $('#reschedule_date').val();
        const newStartTime = $('#reschedule_start_time').val();
        const newEndTime = $('#reschedule_end_time').val();

        $.ajax({
            url: '/reschedule-section',
            method: 'POST',
            data: {
            ...
            success: function(response) {
                if (response.success) {
                    // Utiliser l'ID de la nouvelle section pour mettre à jour l'interface
                    updateAfterReschedule(sectionId, response.new_section_id, newDate, newStartTime, newEndTime);
                    $('#rescheduleSectionModal').modal('hide');
                    Swal.fire('Succès', 'La séance a été reprogrammée', 'success');
                }
            },
            error: function(xhr) {
    ...
    // Fonction pour mettre à jour l'interface après une annulation
    function updateAfterCancel(sectionId, cancellationReason) {
        const sectionContainer = $(`#section-container-${sectionId}`);
        
        // Ajouter la classe et le badge d'annulation
        sectionContainer.addClass('cancelled-section');
        sectionContainer.find('.section-header').prepend(`
            <div class="cancellation-badge">ANNULÉ</div>
            <div class="cancellation-reason">Raison : ${cancellationReason}</div>
        `);
        
        // Mettre à jour les boutons d'action
        const actionButtons = sectionContainer.find('.action-buttons');
        actionButtons.html(`
            <button class="btn btn-sm btn-success restore-section" data-id="${sectionId}">
                <i class="las la-undo-alt"></i> Restaurer
            </button>
            <button class="btn btn-sm btn-danger delete-section" data-id="${sectionId}">
                <i class="las la-trash"></i> Supprimer
            </button>
        `);
        
        // Masquer les éléments d'édition
        sectionContainer.find('.edit-icon, .add-lecture-btn').hide();
    }

    // Fonction pour mettre à jour l'interface après une restauration
    function updateAfterRestore(sectionId) {
        const sectionContainer = $(`#section-container-${sectionId}`);
        
        // Supprimer les classes et badges
        sectionContainer.removeClass('cancelled-section');
        sectionContainer.find('.cancellation-badge, .cancellation-reason').remove();
        
        // Restaurer les boutons d'action originaux
        const actionButtons = sectionContainer.find('.action-buttons');
        actionButtons.html(`
            <button class="btn btn-sm btn-primary add-lecture-btn">
                <i class="las la-plus"></i>Chapitre
            </button>
            <button class="btn btn-sm btn-info add-video-url" data-id="${sectionId}">
                <i class="las la-video"></i> Vidéos enregistrées
            </button>
            <button class="btn btn-sm btn-warning cancel-section" data-id="${sectionId}">
        ...
        // Réactiver les éléments d'édition
        sectionContainer.find('.edit-icon, .add-lecture-btn').show();
    }
    ...
    $('#cancelSectionForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    updateAfterCancel(sectionId, cancellationReason);
    ...
    // Gestionnaire pour la restauration
    $(document).on('click', '.restore-section', function() {
        const sectionId = $(this).data('id');
        
        Swal.fire({
            title: 'Restaurer la séance ?',
            text: 'Voulez-vous vraiment restaurer cette séance ?',
            icon: 'question',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            updateAfterRestore(sectionId);
                            Swal.fire('Succès', 'La séance a été restaurée', 'success');
                        }
                    },
                    error: function(xhr) {
                        Swal.fire('Erreur', 'Une erreur est survenue', 'error');
    ...
    // Ajoutez d'autres gestionnaires pour les autres actions ici...
});
</script>
<div class="page-content">
    ...
    {{ $item->is_rescheduled ? 'rescheduled-section' : '' }}
    ...
    <div class="cancellation-reason">
       Raison : {{ $item->cancellation_reason }}  <!-- Affiche la raison exacte saisie -->
    </div>
@elseif($item->is_rescheduled)
    <div class="rescheduled-badge">REPROGRAMMÉ</div>
@endif
...
@elseif($item->is_rescheduled && !\Carbon\Carbon::parse($item->date)->isPast())
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="rescheduleSectionForm">
                    @csrf
                    <input type="hidden" id="reschedule_section_id" name="section_id">
                    <input type="hidden" id="reschedule_section_title" name="section_title">
                    
                    <div class="mb-3">
                        <label class="form-label">Date</label>
                        <input type="date" name="date" id="reschedule_date" class="form-control" required 
                    ...
                    <div class="mb-3">
                        ...
                        <input type="time" name="start_time" id="reschedule_start_time" class="form-control" required>
                        <div class="error-message text-danger mt-1"></div>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Heure de fin</label>
                        <input type="time" name="end_time" id="reschedule_end_time" class="form-control" required>
                        <div class="error-message text-danger mt-1"></div>
                    </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="las la-calendar-alt me-1"></i>Reprogrammer
                        </button>
                    </div>
                </form>
            </div>
...
$(document).ready(function() {
    ...
    function initializeEventHandlers() {
        // Gestionnaire pour le bouton de reprogrammation
        $(document).on('click', '.reschedule-section', handleRescheduleClick);
        $('#rescheduleSectionForm').on('submit', handleRescheduleSubmit);
    ...
    function handleRescheduleClick() {
        ...
        prefillRescheduleForm($sectionContainer, sectionId);
        
        $('#rescheduleSectionModal').modal('show');
    }

    function prefillRescheduleForm($sectionContainer, sectionId) {
        const currentDate = $sectionContainer.find('.date-text').text().split('/').reverse().join('-');
        const currentStartTime = $sectionContainer.find('.start-time-text').text();
        const currentEndTime = $sectionContainer.find('.end-time-text').text();
        const currentTitle = $sectionContainer.find('.section-title-text').text();

        $('#reschedule_section_id').val(sectionId);
        $('#reschedule_date').val(currentDate);
        $('#reschedule_start_time').val(currentStartTime);
        $('#reschedule_end_time').val(currentEndTime);

        if ($('#original_section_title').length === 0) {
            $('#rescheduleSectionForm').append(
                `<input type="hidden" id="original_section_title" name="original_section_title" value="${currentTitle}">`
            );
        } else {
            $('#original_section_title').val(currentTitle);
        }
    }
    ...
    async function handleRescheduleSubmit(e) {
        ...
        try {
            ...
            if (response.success) {
                updateAfterReschedule(response);
                $('#rescheduleSectionModal').modal('hide');
                Swal.fire('Succès', 'La séance a été reprogrammée', 'success');
            } else {
            ...
            showError('Une erreur est survenue lors de la reprogrammation.');
        }
    }

    function updateAfterReschedule(response) {
        const { original_section_id, new_section_id, new_date, new_start_time, new_end_time } = response;
        const originalSectionContainer = $(`#section-container-${original_section_id}`);
        const newSectionContainer = $(`#section-container-${new_section_id}`);
        ...
        newSectionContainer.find('.date-text').text(new_date);
        newSectionContainer.find('.start-time-text').text(new_start_time);
        newSectionContainer.find('.end-time-text').text(new_end_time);
        newSectionContainer.addClass('rescheduled-section');
        newSectionContainer.find('.section-title-text').append(' (Reprogrammé)');
    }
...
$(document).ready(function() {
    ...
    function initializeEventHandlers() {
        ...
        $('.restore-section').off('click').on('click', function() {
            ...
            Swal.fire({
                title: 'Restaurer la séance ?',
                text: 'Voulez-vous vraiment Restaurercette séance ?',
                icon: 'question',
                showCancelButton: true,
                confirmButtonText: 'Oui, Restaurer',
                cancelButtonText: 'Annuler'
...
<script>
    ...
    const inputType = this.config.fieldType === 'date' ? 'date' : this.config.fieldType;
    const value = this.config.fieldType === 'date' ? this.formatDateForInput(currentValue) : currentValue;
    const today = new Date().toISOString().split('T')[0];
    
    return `<input type="${inputType}" 
                   class="form-control" 
                   value="${value || ''}" 
                   ${inputType === 'date' ? `min="${today}" pattern="\\d{4}-\\d{2}-\\d{2}"` : ''}
                   style="width: auto; display: inline-block;" />`;
}
    ...
    formatDateForInput(dateStr) {
        if (!dateStr) return '';
        // Convertit le format DD/MM/YYYY en YYYY-MM-DD
    ...
    formatDateForDisplay(dateStr) {
        if (!dateStr) return '';
        // Convertit le format YYYY-MM-DD en DD/MM/YYYY
    ...
    validateDate(date) {
        ...
        if (selectedDate < today) {
            throw new Error('Impossible de planifier une séance dans le passé');
        }
    ...
    // Vérifier que les minutes sont par intervalles de 5
    const validateMinutes = (time) => {
        const minutes = parseInt(time.split(':')[1]);
        if (minutes % 5 !== 0) {
            throw new Error('Les minutes doivent être par intervalles de 5 minutes');
        }
    };

    validateMinutes(startTime);
    validateMinutes(endTime);

    if (startTime < minTime) {
        throw new Error('Les séances ne peuvent pas commencer avant 08:00');
    }

    if (endTime > maxTime) {
        throw new Error('Les séances doivent se terminer au plus tard à 23:00');
    }
    ...
    updateFieldDisplay(container, values) {
        if (this.config.fieldType === 'time-range') {
            container.find('.time-range-inputs').remove();
            container.find('.start-time-text').text(values.startTime).show();
            container.find('.end-time-text').text(values.endTime).show();
        } else {
            let displayValue;
            if (this.config.fieldType === 'date') {
                displayValue = this.formatDateForDisplay(values.date);
            } else {
                displayValue = values[this.config.fieldName];
            }
...
$(document).ready(() => {
    ...
    fields.forEach(field => new EditableField(field));
});
</script>
...
.rescheduled-section .section-title-text::after {
    content: " (Reprogrammé)";
    color: #ffc107;
    font-weight: bold;
}
