...
<div class="page-content">
            ...
            <div class="course-section 
                ...
                <div class="section-header" data-section="{{ $item->id }}">
                    ...
                    <div class="section-header-content">
                        <!-- Informations principales de la section -->
    ...
    @if($item->is_cancelled)
        @if(!$item->is_rescheduled)
            <!-- Séance annulée (jamais reprogrammée) -->
        ...
        @else
            <!-- Séance annulée après avoir été reprogrammée -->
            <button class="btn btn-sm btn-danger delete-section" 
                    data-id="{{ $item->id }}" 
                    data-title="{{ $item->section_title }}">
                <i class="las la-trash"></i> Supprimer
    ...
    @elseif($item->is_rescheduled && !\Carbon\Carbon::parse($item->date)->isPast())
        <!-- Séance reprogrammée (peut être annulée et reprogrammée à nouveau) -->
        <button class="btn btn-sm btn-primary add-lecture-btn" 
                onclick="addLecture({{ $course->id }}, {{ $item->id }})">
            <i class="las la-plus"></i> Chapitre
        </button>
        <button class="btn btn-sm btn-info add-video-url" 
                data-id="{{ $item->id }}" 
...
.cancelled-section.rescheduled-section {
    opacity: 0.7;
}

.cancelled-section.rescheduled-section .action-buttons {
    opacity: 1;
}
</style>
@endsection
