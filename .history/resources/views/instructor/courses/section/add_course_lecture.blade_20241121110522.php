...
<script>
    ...
    getFieldValue(container) {
        ...
        if (fieldType === 'date') {
            return container.find('input').val()?.trim() || '';
        }
        return container.find(`.${fieldTextClass}`).text()?.trim() || '';
    }
    ...
    async saveField(container) {
        ...
        try {
            ...
            const newValue = this.config.fieldType === 'date'
    ...
    updateFieldDisplay(container, value) {
        if (this.config.fieldType === 'date') {
            const displayValue = this.formatDateForDisplay(value.value);
