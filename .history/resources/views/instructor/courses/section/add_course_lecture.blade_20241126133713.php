...
<script>
    ...
    var originalData = {
        title: sectionContainer.find('.section-title-text').text(),
        date: sectionContainer.find('.date-text').text(),
        startTime: sectionContainer.find('.start-time-text').text(),
        endTime: sectionContainer.find('.end-time-text').text(),
        link: sectionContainer.find('.section-link').attr('href'),
        chapters: [] // Tableau pour stocker les chapitres
    };

    // Récupérer les chapitres
    sectionContainer.find('.lecture-item').each(function() {
        originalData.chapters.push({
            title: $(this).find('.lecture-title-text').text()
        });
    });

    // Remplir le modal avec les informations
    $('#reschedule_original_section_id').val(sectionId);
    $('#reschedule_section_title').val(originalData.title);
    
    // Convertir la date du format dd/mm/yyyy au format yyyy-mm-dd pour l'input de type date
    var dateParts = originalData.date.split('/');
    var formattedDate = dateParts[2] + '-' + dateParts[1] + '-' + dateParts[0];
    $('input[name="date"]').val(formattedDate);

    $('#reschedule_start_time').val(originalData.startTime);
    $('#reschedule_end_time').val(originalData.endTime);
    $('#reschedule_link').val(originalData.link);

    // Afficher le modal
    $('#rescheduleSectionModal').modal('show');

    // Stocker les données originales pour une utilisation ultérieure si nécessaire
    $('#rescheduleSectionModal').data('originalData', originalData);
});
...
$('#rescheduleSectionForm').on('submit', function(e) {
    ...
    $.ajax({
        ...
        method: 'POST',
        data: $(this).serialize() + '&chapters=' + JSON.stringify(originalData.chapters),
