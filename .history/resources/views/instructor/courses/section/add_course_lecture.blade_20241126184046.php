            ...
            <div class="modal-body">
                <form id="rescheduleSectionForm">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="section_title" id="reschedule_section_title" class="form-control" readonly>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="time" name="start_time" id="reschedule_start_time" class="form-control" readonly>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Heure de fin</label>
                        <input type="time" name="end_time" id="reschedule_end_time" class="form-control" readonly>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Lien de la séance</label>
                        <input type="url" name="link" id="reschedule_link" class="form-control" readonly>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Raison du report</label>
                        <textarea class="form-control" name="reschedule_note" placeholder="Raison du changement de date (optionnel)" rows="3"></textarea>
                    </div>
...
<script>
  $(document).ready(function() {
    ...
    $('.reschedule-section').on('click', function() {
        ...
        // Récupérer les détails actuels de la séance
        var sectionTitle = sectionContainer.find('.section-title-text').text();
        var startTime = sectionContainer.find('.start-time-text').text();
        var endTime = sectionContainer.find('.end-time-text').text();
        var link = sectionContainer.find('.section-link').attr('href');
        
        // Remplir le modal de reprogrammation
        $('#reschedule_section_id').val(sectionId);
        $('#reschedule_section_title').val(sectionTitle);
        $('#reschedule_start_time').val(startTime);
        $('#reschedule_end_time').val(endTime);
        $('#reschedule_link').val(link);
        
        // Afficher le modal
        $('#rescheduleSectionModal').modal('show');
    });

    // Gestionnaire de soumission du formulaire de reprogrammation
    $('#rescheduleSectionForm').on('submit', function(e) {
        e.preventDefault();
        
        // Ici, vous ajouterez l'appel AJAX pour enregistrer la nouvelle date
        $.ajax({
            url: '{{ route("reschedule.course.section") }}', // Vous devrez définir cette route
            method: 'POST',
            data: $(this).serialize(),
            success: function(response) {
                if (response.success) {
                    ...
                    // Optionnellement, recharger la page ou mettre à jour dynamiquement
                    location.reload();
                } else {
                    Swal.fire({
                        icon: 'error',
