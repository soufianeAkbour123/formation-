                ...
                <form id="rescheduleSectionForm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" id="reschedule_date" name="date" class="form-control" required 
                    ...
                    <div class="mb-3">
                        ...
                        <input type="url" id="reschedule_link" name="link" class="form-control" required>
                    </div>

                    <!-- Section pour les lectures -->
                    <div class="mb-3">
                        <h6>Chapitres existants</h6>
                        <div id="reschedule_lectures_container">
                            <!-- Les lectures seront chargées dynamiquement ici -->
                        </div>
                    </div>
...
<script>
 function rescheduleCourse(sectionId) {
    $.ajax({
        url: '/get-section-details/' + sectionId,
        method: 'GET',
        success: function(section) {
            // Remplir les champs principaux
            $('#reschedule_original_section_id').val(section.id);
            $('#reschedule_section_title').val(section.section_title);
            $('#reschedule_date').val(section.date);
            $('#reschedule_start_time').val(section.start_time.substr(0, 5));
            $('#reschedule_end_time').val(section.end_time.substr(0, 5));
            $('#reschedule_link').val(section.link);

            // Charger et afficher les lectures
            var lecturesContainer = $('#reschedule_lectures_container');
                    ...
                    var lectureHtml = `
                        <div class="lecture-item mb-2">
                            <div class="input-group">
                                <input type="text" class="form-control lecture-title" 
                                       value="${lecture.lecture_title}" 
                                       name="lectures[${lecture.id}]">
                                <span class="input-group-text">
                                    <input type="checkbox" class="form-check-input lecture-include" 
                                           checked title="Inclure ce chapitre">
                                </span>
                            </div>
                        </div>
                    `;
                    lecturesContainer.append(lectureHtml);
                });
            } else {
                lecturesContainer.append('<p>Aucun chapitre existant</p>');
            }
...
$('#rescheduleSectionForm').on('submit', function(e) {
    ...
    $('.lecture-item').each(function() {
        var $this = $(this);
        var $checkbox = $this.find('.lecture-include');
        var $titleInput = $this.find('.lecture-title');

        if ($checkbox.is(':checked')) {
            lectures.push({
                id: $titleInput.attr('name').match(/\d+/)[0],
                title: $titleInput.val()
            });
        }
    });

    // Log des lectures avant envoi
    console.log('Lectures à envoyer:', lectures);
    ...
    $.ajax({
        ...
        success: function(response) {
            Swal.fire('Succès', 'Séance reprogrammée avec succès', 'success').then(() => {
                location.reload();
            });
        },
        error: function(xhr) {
            ...
            Swal.fire('Erreur', 'Impossible de reprogrammer la séance: ' + xhr.responseText, 'error');
        }
    });
});

// Attacher l'événement au bouton de reprogrammation
$(document).on('click', '.reschedule-section', function() {
    var sectionId = $(this).data('id');
    rescheduleCourse(sectionId);
});
    </script>
