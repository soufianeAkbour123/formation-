...
unction updateAfterCancel(sectionId, cancellationReason) {
   const sectionContainer = $(`#section-container-${sectionId}`);
   
   // 1. Ajouter la classe et le badge d'annulation
   sectionContainer.addClass('cancelled-section');
   ...
   // 3. Masquer les éléments d'édition
...
unction updateAfterReschedule(sectionId, newData) {
   const sectionContainer = $(`#section-container-${sectionId}`);
   
   // 1. Ajouter la classe de reprogrammation
   sectionContainer.addClass('rescheduled-section');
   
   // 2. Mettre à jour les dates et heures
   sectionContainer.find('.date-text').text(newData.date);
   sectionContainer.find('.start-time-text').text(newData.start_time);
   sectionContainer.find('.end-time-text').text(newData.end_time);
   
   // 3. Mettre à jour les boutons d'action
   const actionButtons = sectionContainer.find('.action-buttons');
   actionButtons.html(`
...
// Gestionnaire pour le formulaire de reprogrammation
('#rescheduleSectionForm').on('submit', function(e) {
   e.preventDefault();
   ...
   const sectionId = $('#reschedule_section_id').val();
    $.ajax({
       ...
       success: function(response) {
           if (response.success) {
               // Mettre à jour l'interface
               updateAfterReschedule(sectionId, response.data);
               ...
               Swal.fire({
                   title: 'Succès',
                   text: 'La séance a été reprogrammée',
                   icon: 'success',
                   timer: 1500
               });
           }
...
unction updateAfterRestore(sectionId) {
   const sectionContainer = $(`#section-container-${sectionId}`);
   
   // 1. Supprimer les classes et badges
   sectionContainer.removeClass('cancelled-section rescheduled-section');
   sectionContainer.find('.cancellation-badge, .cancellation-reason').remove();
   
   // 2. Restaurer les boutons d'action originaux
   const actionButtons = sectionContainer.find('.action-buttons');
   actionButtons.html(`
       <button class="btn btn-sm btn-primary add-lecture-btn">
           <i class="las la-plus"></i>Chapitre
   ...
   // 3. Réactiver les éléments d'édition
