            ...
            <div class="modal-body">
                <form action="{{ route('add.course.section') }}" method="POST" id="addSectionForm">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="date" class="form-control" required pattern="\d{4}-\d{2}-\d{2}">
                        <div class="error-message text-danger mt-1"></div>
                    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (validators.date() && validators.times()) {
        try {
            ...
            console.log('Date envoyée:', formData.get('date'));
            ...
            if (data.success) {
                ...
                if (data.errors) {
                    ...
                    const input = data.message.toLowerCase().includes('créneau') ? 
                        inputs.startTime : 
                        data.message.toLowerCase().includes('date') ? 
                            inputs.date : null;
