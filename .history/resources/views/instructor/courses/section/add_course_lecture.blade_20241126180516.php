...
<script>
 function rescheduleSectionHandler(sectionId) {
    // Récupérer les informations de la séance originale
    $.ajax({
        url: `/get-section-details/${sectionId}`, // Créez cette route dans votre contrôleur
        method: 'GET',
        success: function(section) {
            // Ouvrir le modal d'ajout de séance
            $('#addSectionModal').modal('show');
            
            // Pré-remplir le formulaire avec les informations de la séance originale
            $('#addSectionForm input[name="section_title"]').val(section.section_title + ' (Reprogrammé)');
            $('#addSectionForm input[name="date"]').val(''); // Laisser la date vide pour nouvelle sélection
            $('#addSectionForm input[name="start_time"]').val(section.start_time);
            $('#addSectionForm input[name="end_time"]').val(section.end_time);
            $('#addSectionForm input[name="link"]').val(section.link);
            
            // Ajouter un champ caché pour indiquer la reprogrammation
            if ($('#original_section_id').length === 0) {
                $('#addSectionForm').append(
                    `<input type="hidden" 
                ...
                $('#original_section_id').val(sectionId);
            }
        },
        error: function() {
            Swal.fire({
...
// Gestionnaire d'événement pour le bouton de reprogrammation
$(document).on('click', '.reschedule-section', function() {
    const sectionId = $(this).data('id');
    rescheduleSectionHandler(sectionId);
});

// Modifier le traitement côté serveur (dans votre contrôleur)
// Dans la méthode de création de section, ajoutez la logique suivante:
function addCourseSection(request) {
    $section = new Section();
    ...
    $section->date = $request->input('date');
    $section->start_time = $request->input('start_time');
    $section->end_time = $request->input('end_time');
    $section->link = $request->input('link');
    
    // Réinitialiser les indicateurs d'annulation/report
    $section->is_cancelled = false;
    $section->is_postponed = false;
    $section->cancellation_reason = null;
    
    // Si une section originale est fournie, copier ses lectures
    if ($request->has('original_section_id')) {
        $originalSection = Section::find($request->input('original_section_id'));
        
        // Copier les lectures de la section originale
        foreach ($originalSection->lectures as $originalLecture) {
            $newLecture = $originalLecture->replicate();
            $newLecture->section_id = $section->id;
            $newLecture->save();
        }
    }
