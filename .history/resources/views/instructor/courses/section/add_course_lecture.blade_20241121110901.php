...
<script>
    ...
    async saveField(container) {
        ...
        try {
            ...
            if (this.config.fieldType === 'date') {
                newValues['date'] = container.find('input').val();
    ...
    updateFieldDisplay(container, newValues) {
        if (this.config.fieldType === 'date') {
            const displayValue = this.formatDateForDisplay(newValues.date);
            container.find(`.${this.config.fieldTextClass}`).text(displayValue).show();
            container.find('input').remove();
        } else if (this.config.fieldType === 'time-range') {
            container.find('.time-range-inputs').remove();
            container.find('.start-time-text').text(newValues.startTime).show();
            container.find('.end-time-text').text(newValues.endTime).show();
        } else {
            const displayValue = container.find('input').val();
            container.find(`.${this.config.fieldTextClass}`).text(displayValue).show();
            container.find('input').remove();
        }
