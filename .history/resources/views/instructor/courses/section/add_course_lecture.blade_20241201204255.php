...
<div class="page-content">
    ...
    @if($item->is_cancelled)
        @php
            $rescheduledCount = \App\Models\CourseSection::where('original_section_id', 
                $item->original_section_id ?? $item->id)
                ->where('is_rescheduled', true)
                ->count();
        @endphp
        
        @if($rescheduledCount < 2)
            <!-- Séance qui peut encore être reprogrammée -->
        ...
        @else
            <!-- Séance qui a atteint la limite de reprogrammation -->
            <button class="btn btn-sm btn-danger delete-section" 
                    data-id="{{ $item->id }}">
                <i class="las la-trash"></i> Supprimer
            </button>
