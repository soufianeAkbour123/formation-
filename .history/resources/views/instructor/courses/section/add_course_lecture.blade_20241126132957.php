            ...
            <div class="modal-body">
                <form id="rescheduleSectionForm">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" id="reschedule_date" name="date" class="form-control" required min="<?php echo date('Y-m-d'); ?>">
                    </div>
...
<script>
    ...
    var originalTitle = sectionContainer.find('.section-title-text').text();
    var originalDate = sectionContainer.find('.date-text').text(); // Récupération de la date
    var originalStartTime = sectionContainer.find('.start-time-text').text();
    var originalEndTime = sectionContainer.find('.end-time-text').text();
    var originalLink = sectionContainer.find('.section-link').attr('href');

    // Remplir le modal avec les informations
    $('#reschedule_original_section_id').val(sectionId);
    $('#reschedule_section_title').val(originalTitle);
    ...
    $('#reschedule_start_time').val(originalStartTime);
    $('#reschedule_end_time').val(originalEndTime);
    $('#reschedule_link').val(originalLink);
    ...
    $('#rescheduleSectionForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("reschedule.course.section") }}', // Créez cette route
            method: 'POST',
            data: $(this).serialize(),
