...
$(document).ready(function() {
    ...
    $('.reschedule-section').on('click', function() {
        const sectionId = $(this).data('id');
        const $sectionContainer = $(`#section-container-${sectionId}`);
        ...
        // Ajouter un champ caché pour stocker le titre original
    ...
    // Soumettre le formulaire de reprogrammation
   $('#rescheduleSectionForm').on('submit', function(e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                // Afficher les informations sur les chapitres de la séance d'origine
                let lectureHtml = '';
                response.lectures.forEach(function(lecture) {
                    lectureHtml += `<div>${lecture.lecture_title}</div>`;
                });
                ...
                Swal.fire({
                    ...
                    html: `
                        <p>La séance "${response.original_title}" a été reprogrammée. L'ancienne séance reste marquée comme annulée.</p>
                        <h5>Chapitres de la séance d'origine:</h5>
                        ${lectureHtml}
                    `,
                    footer: '<div class="text-muted">Un nouvel enregistrement a été créé avec un indicateur de reprogrammation.</div>'
                }).then(() => {
                    location.reload();
                });
            } else {
                Swal.fire({
                    ...
                    footer: response.error_details ? `<pre>${JSON.stringify(response.error_details, null, 2)}</pre>` : ''
                });
            }
            $('#rescheduleSectionModal').modal('hide');
        },
    ...
    const errorMessage = xhr.responseJSON 
        ? (xhr.responseJSON.message || 'Une erreur est survenue lors de la reprogrammation.')
        : 'Une erreur est survenue lors de la reprogrammation.';
