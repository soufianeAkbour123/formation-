...
<script>
   ...
   class EditableField {
    ...
    generateInputHtml(currentValue) {
        const inputType = this.config.fieldType === 'date' ? 'date' : this.config.fieldType;
        return `<input type="${inputType}" class="form-control" value="${currentValue || ''}" style="width: auto; display: inline-block;" />`;
    }
    ...
    async startEditing(container) {
        ...
        try {
            ...
            const inputHtml = this.generateInputHtml(currentValue);
    ...
    async saveField(container) {
        ...
        try {
            ...
            const newValue = container.find('input').val();
            formData.append(this.config.fieldName, newValue);
            ...
            this.updateFieldDisplay(container, newValue);
    ...
    updateFieldDisplay(container, newValue) {
        container.find('input').remove();
        container.find(`.${this.config.fieldTextClass}`).text(newValue).show();
    ...
    showInputError(container, message) {
        ...
        container.find('input').after(errorDiv);
