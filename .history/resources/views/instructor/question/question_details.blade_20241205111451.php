...
<div class="page-content">
    <div class="chat-wrapper">
                                ...
                                <a href="{{ route('question.details', $q->id) }}" class="list-group-item">
                                    <div class="d-flex">
                                        ...
                                        <div class="flex-grow-1 ms-2">
                                            ...
                                            <p class="mb-0 chat-msg">{{ Str::limit($q->subject, 30) }}</p>
                                        </div>
        ...
        <div class="chat-content">
            @foreach($allQuestions->where('course_id', $question->course_id)->where('user_id', $question->user_id) as $mainQuestion)
                <div class="chat-content-leftside">
                    <div class="d-flex">
                        <img src="{{ (!empty($mainQuestion->user->photo)) ? url('upload/user_images/'.$mainQuestion->user->photo) : url('upload/no_image.jpg')}}" width="48" height="48" class="rounded-circle" alt="" />
                        <div class="flex-grow-1 ms-2">
                            <p class="mb-0 chat-time">{{ $mainQuestion->subject }}, {{ Carbon\Carbon::parse($mainQuestion->created_at)->diffForHumans() }}</p>
                            <p class="chat-left-msg">{{ $mainQuestion->question }}</p>
                        </div>
                    </div>
                </div>

                @foreach($replay->where('parent_id', $mainQuestion->id) as $rep)
