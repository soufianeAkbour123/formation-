...
<script>
    // Créez un nouveau fichier JavaScript (par exemple screen-protection.js)
document.addEventListener('DOMContentLoaded', function() {
    // Détecter le mode plein écran
    document.addEventListener('fullscreenchange', preventRecording);
    document.addEventListener('webkitfullscreenchange', preventRecording);
    document.addEventListener('mozfullscreenchange', preventRecording);
    document.addEventListener('MSFullscreenChange', preventRecording);
    ...
    window.addEventListener('keydown', function(e) {
        ...
        if (e.keyCode === 44) {
            e.preventDefault();
            alert("La capture d'écran n'est pas autorisée sur cette page.");
        }
        
        // Désactiver les raccourcis de capture d'écran
        if ((e.ctrlKey || e.metaKey) && (e.shiftKey && e.keyCode === 73 || e.keyCode === 67)) {
            e.preventDefault();
    ...
    document.addEventListener('contextmenu', function(e) {
        e.preventDefault();
        alert("Le clic droit n'est pas autorisé sur cette page.");
    });

    // Détecter l'API de capture d'écran
    if (navigator.mediaDevices && navigator.mediaDevices.getDisplayMedia) {
        navigator.mediaDevices.getDisplayMedia = function() {
            return new Promise((resolve, reject) => {
                reject(new Error('La capture d\'écran n\'est pas autorisée sur cette page.'));
            });
        };
    }
});
...
function preventRecording() {
    if (document.fullscreenElement || 
        ...
        // Ajouter une classe CSS qui applique un flou ou autre effet
...
function showWarningMessage() {
    let warning = document.getElementById('recording-warning');
    if (!warning) {
        ...
        warning.innerHTML = 'L\'enregistrement d\'écran n\'est pas autorisé sur cette page.';
...
.recording-protected video,
.recording-protected iframe,
.recording-protected img {
    visibility: hidden;
}
