...
<script>
  // screen-recording-protection.js
...
// 2. Détecter et bloquer la capture d'écran
document.addEventListener('keydown', (e) => {
    // Bloquer les combinaisons de touches de capture d'écran
    if (
        (e.ctrlKey && e.shiftKey && (e.key === 'I' || e.key === 'i')) || // Chrome dev tools
        (e.ctrlKey && (e.key === 'u' || e.key === 'U')) || // View source
        (e.key === 'F12') || // Dev tools
        (e.ctrlKey && (e.key === 'c' || e.key === 'C')) || // Copy
        (e.ctrlKey && (e.key === 'P' || e.key === 'p')) || // Print
        (e.ctrlKey && (e.key === 'k' || e.key === 'K')) || // Performance tools
        (e.metaKey && (e.key === 'p' || e.key === 'P')) || // macOS print
        (e.metaKey && (e.key === 'c' || e.key === 'C')) // macOS copy
    ) {
        e.preventDefault();
        return false;
    }
...
// 3. Détecter l'enregistrement d'écran via l'API MediaDevices
async function detectScreenRecording() {
    try {
        const stream = await navigator.mediaDevices.getDisplayMedia();
        
        // Si l'utilisateur démarre un enregistrement d'écran
        stream.getVideoTracks()[0].onended = () => {
            console.log("L'enregistrement d'écran a été arrêté");
        };
        ...
        alert("L'enregistrement d'écran n'est pas autorisé sur cette page");
        
    } catch (err) {
        console.log("L'utilisateur a refusé l'accès à l'enregistrement d'écran");
    }
}

// 4. Observer les modifications du DOM pour protéger le contenu
const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
        if (mutation.type === 'childList') {
            const node = mutation.target;
            // Empêcher le copier-coller du contenu
...
// 5. Démarrer l'observation du DOM
observer.observe(document.body, {
    childList: true,
    subtree: true
});

// 6. Bloquer l'API Picture-in-Picture
