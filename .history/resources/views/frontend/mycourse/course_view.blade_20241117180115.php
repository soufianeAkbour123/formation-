...
@foreach ($section as $sec)
    <div class="card">
        <div class="card-header p-0" id="headingOne{{ $sec->id }}">
            <button class="btn btn-link w-100 d-flex justify-content-between align-items-center text-left px-3 py-2" 
                    type="button" 
                    data-toggle="collapse"
                    data-target="#collapseOne{{ $sec->id }}"
                    aria-expanded="true"
                    aria-controls="collapseOne">
                <span class="text-dark">{{ $sec->section_title }}</span>
                <i class="la la-angle-down"></i>
            </button>
        </div>
        <div id="collapseOne{{ $sec->id }}" 
            ...
            <div class="card-body p-0">
                <ul class="list-unstyled m-0">
                    <li class="px-3 py-2">
                        @if($sec->video_url)
                            <span data-video-url="{{ $sec->video_url }}">
                                {{ $sec->video_url }}
                            </span>
                        @endif
                    </li>
