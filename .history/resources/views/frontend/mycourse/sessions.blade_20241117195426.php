...
<style>
    ...
    /* Styles améliorés pour les séances annulées */
    .cancelled-session {
        opacity: 0.7;
        background-color: #f3f4f6;
        position: relative;
    ...
    .cancelled-session .section-title {
        color: #6b7280;
    }

    .cancelled-session .status-indicator {
        background: linear-gradient(45deg, #ef4444, #f87171);
    }

    .cancelled-session .schedule-badge,
    .cancelled-session .stat-item {
        opacity: 0.7;
        text-decoration: line-through;
    }

    .cancelled-session .badge.bg-danger {
    ...
    /* Style pour la raison d'annulation */
    .cancellation-reason {
        color: #ef4444;
        font-size: 0.875rem;
        margin-top: 0.25rem;
...
<div class="course-timeline">
    ...
    @php
        $sessionEndTime = \Carbon\Carbon::parse($sec->end_time);
        $isPastSession = $sessionEndTime->lt($now);
    @endphp
    <div class="timeline-section {{ $sec->is_cancelled ? 'cancelled-session' : '' }} {{ $isPastSession && !$sec->is_cancelled ? 'past-session' : '' }}">
        <div class="timeline-header" data-toggle="collapse" data-target="#collapse{{ $sec->id }}" aria-expanded="false">
            <div class="section-status">
                <div class="status-indicator">
                    <i class="la {{ $sec->is_cancelled ? 'la-ban' : ($isPastSession ? 'la-check' : 'la-book-open') }}"></i>
                </div>
        ...
        <div class="section-meta">
            ...
            <div class="schedule-badges justify-content-end">
                <span class="schedule-badge">
                    ...
                    {{ \Carbon\Carbon::parse($sec->start_time)->format('d/m/Y') }}
                </span>
                <span class="schedule-badge">
                    <i class="la la-clock"></i>
                    {{ \Carbon\Carbon::parse($sec->start_time)->format('H:i') }} - 
                    {{ \Carbon\Carbon::parse($sec->end_time)->format('H:i') }}
                </span>
            </div>
        </div>
