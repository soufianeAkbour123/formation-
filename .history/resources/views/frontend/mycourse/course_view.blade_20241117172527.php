...
@foreach ($section as $sec)
    <div class="card">
        ...
        <div id="collapseOne{{ $sec->id }}" 
             class="collapse" 
             aria-labelledby="headingOne{{ $sec->id }}" 
             data-parent="#accordionCourseExample">
            <div class="card-body p-0">
                <p class="text-muted p-3">
                    Contenu de la section : {{ $sec->description ?? 'Aucune description disponible.' }}
                </p>
                <div id="videoContainer{{ $sec->id }}" class="video-container p-3" style="display: none;">
                    <!-- Le conteneur vidéo sera rempli par JavaScript -->
                </div>
            </div><!-- end card-body -->
        </div><!-- end collapse -->
    </div><!-- end card -->
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.section-checkbox').forEach(function (checkbox) {
        checkbox.addEventListener('change', function () {
            ...
            const sectionId = this.id.replace('checkbox', '');
            const videoContainer = document.getElementById('videoContainer' + sectionId);
            
            // Décocher toutes les autres checkboxes
            document.querySelectorAll('.section-checkbox').forEach(function (cb) {
                if (cb !== checkbox) {
                    cb.checked = false;
                    const otherId = cb.id.replace('checkbox', '');
                    const otherContainer = document.getElementById('videoContainer' + otherId);
                    if (otherContainer) {
            ...
            if (this.checked && videoUrl) {
                videoContainer.style.display = 'block';
                // Vous pouvez personnaliser l'affichage de la vidéo selon vos besoins
                // Par exemple, avec une iframe YouTube ou un lecteur video HTML5
    ...
    // Conserver la fonctionnalité d'expansion/réduction
    document.querySelectorAll('.section-title').forEach(function (button) {
