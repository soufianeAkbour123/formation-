...
<div class="card">
    <div class="card-header" id="headingOne{{ $sec->id }}">
        <div class="course-item-content-wrap">
            ...
            <button class="btn btn-link section-title" type="button" data-toggle="collapse" 
    ...
    <div id="collapseOne{{ $sec->id }}" class="collapse" aria-labelledby="headingOne{{ $sec->id }}" data-parent="#accordionCourseExample">
        <div class="card-body p-0">
            <ul class="curriculum-sidebar-list">
                @foreach ($sec->lectures as $lecture)
                    <li class="course-item-link active">
                        <div class="course-item-content">
                            <h4 class="fs-15">{{ $lecture->lecture_title }}</h4>
                        </div>
                    </li>
                @endforeach
            </ul>
...
document.addEventListener('DOMContentLoaded', function() {
    // Ajouter le gestionnaire d'événements pour les titres de section
    const sectionTitles = document.querySelectorAll('.section-title');
    sectionTitles.forEach(title => {
        title.addEventListener('click', function() {
            const videoUrl = this.getAttribute('data-video-url');
            if (videoUrl) {
                // Ici, vous pouvez ajouter le code pour gérer la lecture de la vidéo
                console.log('Video URL:', videoUrl);
                // Par exemple : showVideo(videoUrl);
            }
        });
    });
});
