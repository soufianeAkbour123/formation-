...
$isPastSession = $sessionDate->isToday() && $sessionEndTime->isPast();
// Vérifie si la session est en cours
$isOngoingSession = $sessionDate->isToday() && 
                    $now->between($sessionStartTime, $sessionEndTime);
@endphp
<div class="timeline-section {{ $sec->is_cancelled ? 'cancelled-session' : '' }} {{ $isPastSession ? 'past-session' : '' }}">
        ...
        @if(!$sec->is_cancelled)
            ...
            @else
                @if($sec->video_url)
                <a href="#" 
   class="modern-btn modern-btn-recording view-recording-btn" 
    ...
    recordingButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            if (videoUrl) {
                ...
                // Option 2: If you want to show in a modal or custom viewer
                // openVideoModal(videoUrl);
            } else {
                alert('Aucune vidéo disponible pour cette section.');
            }
        });
    });
});
