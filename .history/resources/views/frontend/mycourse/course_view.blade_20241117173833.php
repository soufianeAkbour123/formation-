...
@foreach ($section as $sec)
    ...
    <div class="card">
        <div class="card-header" id="headingOne{{ $sec->id }}">
            <button class="btn btn-link section-title-btn" type="button" data-toggle="collapse" 
                    data-target="#collapseOne{{ $sec->id }}" aria-expanded="true" 
                    aria-controls="collapseOne" data-video-url="{{ $sec->video_url }}">
                <i class="la la-angle-down"></i>
                <i class="la la-angle-up"></i>
                ...
                <ul class="curriculum-sidebar-list">
                    @foreach ($lectures as $lect)
                        <li class="course-item-link active">
                            <div class="course-item-content-wrap">
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input" id="courseCheckbox{{ $lect->id }}" required>
                                    <label class="custom-control-label custom--control-label" for="courseCheckbox{{ $lect->id }}"></label>
                                </div>
                                <div class="course-item-content">
                                    <h4 class="fs-15 lecture-title" data-video-url="{{ $lect->video_url }}" data-content="{!! $lect->content !!}">
                                        {{ $lect->lecture_title }}
                                    </h4>
                                </div>
                            </div>
...
<script>
    ...
    // Gestion des clics sur les titres de section
    const sectionButtons = document.querySelectorAll('.section-title-btn');
    sectionButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            // Empêcher la propagation si on clique sur les icônes
    ...
    // Gestion des clics sur les titres des conférences
    const lectureTitles = document.querySelectorAll('.lecture-title');
