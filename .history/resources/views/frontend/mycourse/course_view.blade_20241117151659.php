...
@endphp
    ...
    <li class="course-item-link active">
        <div class="course-item-content-wrap">
            <div class="custom-control custom-checkbox">
                <input type="checkbox" class="custom-control-input" id="courseCheckbox{{ $section->id }}" required>
                <label class="custom-control-label custom--control-label" for="courseCheckbox{{ $section->id }}"></label>
            </div><!-- end custom-control -->
            <div class="course-item-content">
                <h4 class="fs-15 section-title" data-video-url="{{ $section->video_url }}">
                    {{ $section->section_title }}
                </h4>
                <p>URL de la vidéo : <a href="{{ $section->video_url }}" target="_blank">{{ $section->video_url }}</a></p> <!-- Affichage de l'URL de la vidéo -->
            </div><!-- end course-item-content -->
        </div><!-- end course-item-content-wrap -->

        <!-- Affichage des lectures associées -->
        <ul>
            @foreach ($section->lectures as $lect)
                <li class="lecture-item">
                    <h5 class="fs-15 lecture-title" data-content="{!! $lect->content !!}">
                        {{ $lect->lecture_title }}
                    </h5>
                </li>
            @endforeach
