...
<div class="mobile-course-menu pt-4">
    <div class="accordion generic-accordion generic--accordion" id="mobileCourseAccordionCourseExample">
        @foreach ($section as $sec)
            <div class="card">
                <div class="card-header" id="mobileCourseHeading{{ $sec->id }}">
                    <button class="btn btn-link" type="button" data-toggle="collapse" 
                            data-target="#mobileCourseCollapse{{ $sec->id }}"
                            aria-expanded="true" aria-controls="mobileCourseCollapse{{ $sec->id }}">
                        <i class="la la-angle-down"></i>
                        <i class="la la-angle-up"></i>
                        <span class="fs-15">{{ $sec->section_title }}</span>
                    </button>
                </div>
                <div id="mobileCourseCollapse{{ $sec->id }}" class="collapse show"
                     aria-labelledby="mobileCourseHeading{{ $sec->id }}"
