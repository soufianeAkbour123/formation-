...
<div class="course-timeline">
    @foreach ($sections->sortBy('date') as $sec)
    @php
        $now = Carbon::now();
        $sessionDate = Carbon::parse($sec->date);
        $sessionStartTime = Carbon::parse($sec->start_time);
        $sessionEndTime = Carbon::parse($sec->end_time);

        // Combine la date et l'heure pour une comparaison précise
        $sessionStart = Carbon::parse($sec->date . ' ' . $sec->start_time);
        $sessionEnd = Carbon::parse($sec->date . ' ' . $sec->end_time);

        // Vérifie si la session est passée (date passée OU même jour mais heure passée)
        $isPastSession = $now->gt($sessionEnd);
        
        // Vérifie si la session est en cours
        $isOngoingSession = $now->between($sessionStart, $sessionEnd);
    @endphp
    <div class="timeline-section 
        {{ $sec->is_cancelled ? 'cancelled-session' : '' }} 
        {{ $sec->is_rescheduled ? 'rescheduled-session' : '' }}
        ...
        <div class="timeline-header" data-toggle="collapse" data-target="#collapse{{ $sec->id }}" aria-expanded="false">
            <div class="section-status">
                <div class="status-indicator">
                    <i class="la 
                        {{ $sec->is_cancelled ? 'la-ban text-danger' : 
                           ($sec->is_rescheduled ? 'la-calendar-alt text-warning' : 
                           ($isPastSession ? 'la-check' : 'la-book-open')) }}"></i>
                </div>
                <h3 class="section-title">
                    ...
                    @elseif($sec->is_rescheduled)
                        <span class="badge bg-warning">Reprogrammée</span>
            ...
            <div>
                @if(!$sec->is_cancelled)
                    @if($isOngoingSession)
                        ...
                        @else
                            <span class="waiting-recording">Le lien de la session sera bientôt disponible</span>
                        @endif
                    @elseif(!$isPastSession)
                        @if($sec->link)
                            <a href="{{ $sec->link }}" class="modern-btn join-btn" target="_blank">Rejoindre</a>
                        @else
                            <span class="waiting-recording">Le lien de la séance sera bientôt disponible</span>
                        @endif
                    @else
                        @if($sec->video_url)
