...
<div class="course-sidebar bg-white shadow-sm">
    <!-- Bouton d'ouverture mobile -->
    <button class="sidebar-toggle-btn md:hidden fixed top-4 left-4 bg-primary text-white rounded-lg p-2 z-50" type="button">
        <i class="la la-angle-left mr-2"></i>
        <span>Contenu du cours</span>
    </button>

    <!-- Contenu principal de la sidebar -->
    <div class="sidebar-content h-full overflow-auto">
        <!-- En-tête -->
        <div class="sidebar-header flex items-center justify-between p-4 border-b">
            <h3 class="text-lg font-semibold">Contenu du cours</h3>
            <button class="close-btn text-gray-500 hover:text-gray-700 md:hidden" type="button">
                <i class="la la-times text-xl"></i>
            </button>
        </div>

        <!-- Sections du cours -->
        <div class="course-sections p-4">
            <div class="accordion" id="courseSections">
                @foreach ($section as $sec)
                <div class="section-item mb-4 border rounded-lg overflow-hidden">
                    <!-- En-tête de section -->
                    <div class="section-header bg-gray-50" id="heading{{ $sec->id }}">
                        <button class="section-toggle w-full flex items-center justify-between p-4 hover:bg-gray-100 transition-colors" 
                                type="button" 
                                data-toggle="collapse" 
                                data-target="#collapse{{ $sec->id }}"
                                aria-expanded="true"
                                aria-controls="collapse{{ $sec->id }}">
                            <span class="font-medium">{{ $sec->section_title }}</span>
                            <i class="la la-angle-down transform transition-transform"></i>
                        </button>
                    </div>

                    <!-- Contenu de section -->
                    <div id="collapse{{ $sec->id }}" 
                         class="section-content collapse show" 
                         aria-labelledby="heading{{ $sec->id }}" 
                         data-parent="#courseSections">
                        <div class="p-4">
                            <div class="lecture-item flex items-center p-2 rounded hover:bg-gray-50 cursor-pointer"
                                 data-video-url="{{ $sec->video_url }}">
                                <i class="la la-play-circle text-primary mr-3"></i>
                                <span class="text-sm">{{ $sec->section_title }}</span>
                            </div>
                        </div>
                    </div>
                </div>
...
.section-toggle[aria-expanded="true"] .la-angle-down {
