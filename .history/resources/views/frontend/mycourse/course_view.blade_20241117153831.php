    ...
    <div class="card-header" id="headingOne{{ $sec->id }}">
        <div class="d-flex align-items-center">
            <div class="custom-control custom-checkbox mr-3">
                <input type="checkbox" class="custom-control-input" id="sectionCheckbox{{ $sec->id }}" required>
                <label class="custom-control-label custom--control-label" for="sectionCheckbox{{ $sec->id }}"></label>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // Ajouter des écouteurs d'événements pour tous les titres de section
    const sectionTitles = document.querySelectorAll('.section-title');
    
    sectionTitles.forEach(title => {
        title.addEventListener('click', function(e) {
            // Empêcher la propagation si on clique sur le checkbox
            if (e.target.closest('.custom-control')) {
                return;
            }
            
            const videoUrl = this.getAttribute('data-video-url');
            if (videoUrl) {
                // Ici, ajoutez votre logique pour afficher la vidéo
                // Par exemple, si vous avez un lecteur vidéo avec un ID spécifique :
    ...
    checkboxes.forEach(checkbox => {
        checkbox.addEventListener('change', function(e) {
            // Empêcher la propagation pour éviter de déclencher l'événement du titre
            e.stopPropagation();
            // Ajoutez ici votre logique pour gérer l'état des checkbox
        });
    });
});
</script>
