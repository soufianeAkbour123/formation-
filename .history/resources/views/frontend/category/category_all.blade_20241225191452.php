    ...
    <div class="container">
        <div class="breadcrumb-content d-flex flex-wrap align-items-center justify-content-between">
            <div class="section-heading">
                <h2 class="section__title text-white">{{ $category->category_name ?? 'Nos Cours' }}</h2>
            </div>
            <ul class="generic-list-item generic-list-item-white generic-list-item-arrow d-flex flex-wrap align-items-center">
                <li><a href="{{ url('/') }}">Accueil</a></li>
                <li>{{ $category->category_name ?? 'Cours' }}</li>
            </ul>
        </div>
    </div>
</section>
                ...
                <div class="sidebar mb-5">
                            ...
                            <form method="get" action="{{ route('search.courses') }}">
                                <div class="form-group mb-0">
                                    <input class="form-control form--control pl-3" type="text" 
                                           name="search" placeholder="Rechercher des cours"
                    ...
                    <div class="card card-item">
                        <div class="card-body">
                            ...
                            <div class="divider"><span></span></div>
                            @foreach(['all' => 'Tous les niveaux', 'Begginer' => 'Débutant', 
                                    'Middle' => 'Intermédiaire', 'Advance' => 'Expert'] as $key => $label)
                                <div class="custom-control custom-checkbox mb-1 fs-15">
                                    <input type="checkbox" class="custom-control-input" 
                                           id="level{{ $key }}" 
                                           onclick="window.location.href='{{ route('filter.level', $key) }}'"
                            ...
                            @foreach([5,4,3,2,1] as $rating)
                                <div class="custom-control custom-radio mb-1 fs-15">
                                    <input type="radio" class="custom-control-input" 
                                           id="{{ $rating }}StarRating" name="rating"
                                           onclick="window.location.href='{{ route('filter.rating', $rating) }}'"
                                           {{ request()->segment(3) == $rating ? 'checked' : '' }}>
                                    <label class="custom-control-label custom--control-label" 
                                           for="{{ $rating }}StarRating">
...
document.querySelectorAll('.level-filter').forEach(checkbox => {
    checkbox.addEventListener('change', function() {
        window.location.href = '/filter/level/' + this.value;
    });
});

document.querySelectorAll('.rating-filter').forEach(radio => {
    radio.addEventListener('change', function() {
        window.location.href = '/filter/rating/' + this.value;
    });
});
</script>
@endpush
