...
<div class="card-body">
    ...
    <div class="divider"><span></span></div>
    @foreach(['Beginner', 'Intermediate', 'Expert'] as $label)
    <div class="custom-control custom-checkbox mb-1 fs-15">
        <input type="checkbox" class="custom-control-input level-filter"
               id="level{{$label}}" value="{{$level}}"
               {{ request()->level == $label ? 'checked' : '' }}>
        <label class="custom-control-label" for="level{{$level}}">
            {{$level}}
            <span class="ml-1 text-gray">
                ({{ App\Models\Course::where('label', $lebel)->count() }})
            </span>
        </label>
    </div>
    @endforeach
