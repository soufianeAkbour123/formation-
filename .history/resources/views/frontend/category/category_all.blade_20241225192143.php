                ...
                <div class="sidebar mb-5">
                    <div class="card card-item">
                        <div class="card-body">
                            ...
                            <form method="get" action="{{ route('search.courses') }}">
                                <div class="form-group mb-0">
                                    <input class="form-control form--control pl-3" 
                                           type="text" name="search" 
                                           placeholder="Rechercher des cours"
                                           value="{{ request('search') }}">
                                    <button type="submit" class="la la-search search-icon border-0 bg-transparent"></button>
                                </div>
                            </form>
</div>
                    </div><!-- end card -->
                            ...
                            @foreach([
                                ...
                                <div class="custom-control custom-checkbox mb-1 fs-15">
                                    <input type="checkbox" class="custom-control-input level-filter" 
                                           id="level{{ $key }}" value="{{ $key }}"
                                           {{ request()->segment(3) == $key ? 'checked' : '' }}>
                                    <label class="custom-control-label custom--control-label text-black" 
                                           for="level{{ $key }}">
                                        {{ $label }}<span class="ml-1 text-gray">({{ $levels[$key] }})</span>
                                    </label>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="card card-item">
                    @php
    $allLevels = ['Beginner' => 'Débutant', 'Middle' => 'Intermédiaire', 'Advance' => 'Expert'];
    $courseCounts = [];
    $totalCourses = 0;
    
    foreach ($allLevels as $key => $level) {
        $count = App\Models\Course::where('label', $key)->count();
...
<div class="card-body">
    ...
    <div class="custom-control custom-checkbox mb-1 fs-15">
        <input type="checkbox" class="custom-control-input level-filter" 
               id="levelCheckboxAll" name="level[]" value="all"
               {{ request()->level == null ? 'checked' : '' }}>
        <label class="custom-control-label custom--control-label text-black" for="levelCheckboxAll">
            Tous les niveaux<span class="ml-1 text-gray">({{ $totalCourses }})</span>
        </label>
    </div>
    ...
    <div class="custom-control custom-checkbox mb-1 fs-15">
        <input type="checkbox" class="custom-control-input level-filter" 
               id="levelCheckbox{{ $key }}" name="level[]" value="{{ $key }}"
               {{ request()->level == $key ? 'checked' : '' }}>
        <label class="custom-control-label custom--control-label text-black" for="levelCheckbox{{ $key }}">
            {{ $level }}<span class="ml-1 text-gray">({{ $courseCounts[$key] }})</span>
        </label>
    </div>
    @endforeach
</div>
...
document.querySelectorAll('.level-filter').forEach(checkbox => {
    checkbox.addEventListener('change', function() {
        if (this.value === 'all') {
            document.querySelectorAll('.level-filter:not([value="all"])').forEach(cb => {
                cb.checked = false;
            });
        } else {
            document.getElementById('levelCheckboxAll').checked = false;
        }
        this.form.submit();
    });
});
