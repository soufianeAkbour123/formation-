...
@php
    $section = App\Models\CourseSection::where('course_id', $course->id)
        ->where(function($query) {
            $query->where('is_cancelled', 0)  // Sessions non annulées
                  ->orWhere(function($q) {    // OU sessions annulées mais reprogrammées
                            ...
                            <div class="ml-2">
                                <div class="section-title">
                                    ...
                                    @elseif($sec->is_rescheduled)
                                        <span class="badge badge-warning">Reprogrammée</span>
                                    @endif
                                </div>
                                @if($sec->cancellation_reason)
                                    <small class="text-muted">Raison: {{ $sec->cancellation_reason }}</small>
                                @endif
                        </button>
                    </div>
                    <!-- Ajout de l'attribut data-parent -->
