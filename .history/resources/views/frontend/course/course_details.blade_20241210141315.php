...
$instructorReviews = App\Models\Review::whereHas('course', function($query) use ($course) {
    $query->where('user_id', $course->instructor->id);
})->where('status', 1)->latest()->get();

// Calculer la moyenne des notes pour tous les cours de l'instructeur
$averageRating = App\Models\Review::whereHas('course', function($query) use ($course) {
    $query->where('user_id', $course->instructor->id);
