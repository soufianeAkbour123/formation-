...
@php
    // Récupère uniquement les sections dans l'état initial (is_cancelled = 0 ET is_rescheduled = 0)
    $sections = App\Models\CourseSection::where('course_id', $course->id)
        ->where('is_cancelled', 0)
        ->where('is_rescheduled', 0)  // Ajoute cette condition pour exclure les sections reprogrammées
        ->orderBy('date', 'asc')
        ->get();
@endphp
