...
<section class="breadcrumb-area pt-50px pb-50px bg-white pattern-bg">
   ...
   {{ round($avarage,1) }}&nbsp;&nbsp;
                    <div class="rating-wrap d-flex flex-wrap align-items-center">
                        ...
                        <span class="rating-total pl-1">{{ count($reviewcount) }} Évaluations</span>
    @php
        $enrollmentCount = App\Models\Order::where('course_id',$course->id)->count();
    @endphp
                        <span class="student-total pl-2">sur {{ number_format($enrollmentCount) }} Apprenants</span>
                    </div>
                </div><!-- end d-flex -->
    ...
    <span style="color: #002366; font-weight: bold;">
    ...
    <b>Date Fin :</b> {{ \Carbon\Carbon::parse($course->date_fin)->format('d/m/Y') }}
                ...
                <div class="bread-btn-box pt-3">
                    ...
                    <button class="btn theme-btn theme-btn-sm theme-btn-transparent lh-28 mr-2 mb-2" data-toggle="modal" data-target="#shareModal">
                        <i class="la la-share mr-1"></i>partagez
                    </button>
...
<div class="course-overview-card border border-gray p-4 rounded">
            ...
            <div class="media media-card">
                <div class="instructor-img">
                    ...
                    <ul class="generic-list-item pt-3">
                        <li><i class="la la-star mr-2 text-color-3"></i> {{ round($avarage,1) }} Instructor Avis</li>
                    
                        <li><i class="la la-comment-o mr-2 text-color-3"></i>3 Avis</li>
                ...
                <div class="media-body">
                    ...
                    <a class="collapse-btn collapse--btn fs-15" data-toggle="collapse" href="#collapseMoreTwo" role="button" aria-expanded="false" aria-controls="collapseMoreTwo">
                        <span class="collapse-btn-hide">Voir plus<i class="la la-angle-down ml-1 fs-14"></i></span>
                        <span class="collapse-btn-show">Voir Moins<i class="la la-angle-up ml-1 fs-14"></i></span>
                    </a>
                </div>
            </div>
                   ...
                   <div class="course-overview-card pt-4">
                       <h3 class="fs-24 font-weight-semi-bold pb-40px">Retour des Apprenants</h3>
   ...
   <div class="skillbar" data-percent="{{ $ratingInfo['percent'] }}%">
        ...
        @endif
                   ...
                   <div class="course-overview-card pt-4">
                       <h3 class="fs-24 font-weight-semi-bold pb-4">Avis</h3>
   ...
   @endphp
    ...
    <div class="media media-card border-bottom border-bottom-gray pb-4 mb-4">
        ...
        <div class="media-body">
            ...
            <p class="pb-2"><b>Avis :</b> {{ $item->comment }}</p>
    ...
    @endforeach
                       ...
                       <div class="see-more-review-btn text-center">
                           <button type="button" class="btn theme-btn theme-btn-transparent">Charger plus d'avis</button>
                       </div>
                   </div><!-- end course-overview-card -->
    ...
    <div class="alert alert-info">
        <p><b>Pour ajouter un avis sur le cours, vous devez d'abord vous connecter. 
            <a href="{{ route('login') }}">Connectez-vous ici</a></b></p>
    </div>
@else
    @php
        $isEnrolled = \App\Models\Order::where('course_id', $course->id)
    ...
    @if(!$isEnrolled)
        <div class="alert alert-warning">
            <p><b>Pour laisser un avis sur ce cours, vous devez d'abord vous y inscrire.</b></p>
            <div class="mt-3">
                <a href="" class="btn btn-primary">Découvrir nos formations</a>
            </div>
        </div>
    @else
        @php
            $attendedSessions = \App\Models\CourseSection::where('course_id', $course->id)
        ...
        @if($attendedSessions < 3)
            <div class="alert alert-warning">
                <p><b>Vous devez avoir assisté à au moins trois séances pour laisser un avis.</b></p>
                <p>Actuellement, vous avez assisté à {{ $attendedSessions }} séance(s).</p>
            </div>
        @else
            @if(session('success'))
            ...
            <div class="course-overview-card pt-4">
                <h3 class="fs-24 font-weight-semi-bold pb-4">Ajouter un avis</h3>
                ...
                <form method="post" action="{{ route('store.review') }}" class="row">
                    ...
                    <div class="input-box col-lg-12">
                        ...
                        <div class="form-group">
                            <textarea class="form-control form--control pl-3" 
                                name="comment" 
                    ...
                    <div class="btn-box col-lg-12">
                        <button class="btn theme-btn" type="submit">Soumettre l'avis</button>
                    </div>
                </form>
            </div>
        @endif
        ...
        <div class="related-course-wrap">
            ...
            <div class="view-more-carousel-2 owl-action-styled">
                ...
                @php
                    ...
                    $reviewcount = App\Models\Review::where('course_id',$inscourse->id)->where('status',1)->latest()->get();
                    $avarage = App\Models\Review::where('course_id',$inscourse->id)->where('status',1)->avg('rating');
                @endphp
                <div class="card card-item">
                    ...
                    <div class="card-body">
                        <h6 class="ribbon ribbon-blue-bg fs-14 mb-3">
                            @if ($inscourse->label == 'Begginer')
                                Débutant
                            @elseif ($inscourse->label == 'Middle')
                                Intermédiaire
                            @elseif ($inscourse->label == 'Advance')
                                Avancé
                            @else
                                {{ $inscourse->label }}
                            @endif
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            @else
                                <p class="card-price text-black font-weight-bold">{{ $inscourse->discount_price }} DH
                                    <span class="before-price font-weight-medium">{{ $inscourse->selling_price }} DH</span>
                                </p> 
                            @endif
...
<style>
    .related-course-area .view-more-carousel-2 {
    display: flex;
    flex-wrap: wrap;
    gap: 20px; /* Optional: space between cards */
}

.related-course-area .card-item {
    flex: 1 1 25%; /* 4 cards per row, adjust as needed */
    min-width: 250px; /* Minimum width to maintain card size */
    display: flex;
    flex-direction: column;
    height: 100%;
    background-color: white;
...
.related-course-area .card-image {
...
.related-course-area .card-image img {
    object-fit: cover; /* Ensures the image covers the container without distortion */
    width: 100%; /* Full width of the parent container */
    height: 100%; /* Full height of the parent container */
}

.related-course-area .card-body {
    display: flex;
    flex-direction: column;
    justify-content: space-between;
...
.related-course-area .card-body > .rating-wrap, 
.related-course-area .card-body > .card-price {
    margin-top: auto; /* Pushes rating and price to the bottom */
}

.related-course-area .card-title, 
.related-course-area .card-text {
