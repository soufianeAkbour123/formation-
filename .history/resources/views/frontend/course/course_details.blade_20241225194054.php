...
<div class="course-overview-card border border-gray p-4 rounded">
   ...
   @endphp
    ...
    @endforeach
                   ...
                   @guest
 <p><b>Pour ajouter un avis sur le cours, vous devez d'abord vous connecter. <a href="{{ route('login') }}">Connectez-vous ici</a></b></p>
else
 @php
     ...
     $isEnrolled = $userId ? App\Models\Order::where('user_id', $userId)
                                 ->where('course_id', $course->id)
                                 ->where('status', '1') // Assurez-vous que la commande est validée
                                 ->exists() : false;
      
     // Si inscrit, compter les séances passées
     $completedSessions = 0;
     if ($isEnrolled) {
         $currentDate = now();
         $completedSessions = App\Models\CourseSection::where('course_id', $course->id)
     ...
     // dd($isEnrolled, $completedSessions);
 @endphp
  @if (!$isEnrolled)
     <div class="alert alert-warning">
         <p><strong>Vous n'êtes pas encore inscrit à ce cours.</strong></p>
         <p>Pour pouvoir laisser un avis, vous devez d'abord vous inscrire à une formation.</p>
         <a href="" class="btn theme-btn">Découvrir nos formations</a>
     </div>
 @elseif ($completedSessions < 3)
     <div class="alert alert-info">
         <p><strong>Vous ne pouvez pas encore laisser un avis.</strong></p>
         <p>Il faut avoir participé à au moins 3 séances pour pouvoir donner votre avis sur ce cours.</p>
         <p>Séances complétées : {{ $completedSessions }}/3</p>
     </div>
 @else
     <div class="course-overview-card pt-4">
         ...
         @endphp
          @if($existingReview)
             <div class="alert alert-info">
                 <p><strong>Vous avez déjà laissé un avis pour ce cours.</strong></p>
                 <p>Vous ne pouvez laisser qu'un seul avis par cours.</p>
             </div>
         @else
